...
__asm__ (\
	...
	"mov %%ax, %%ds;"  					/* 初始化段寄存器指向本局部表数据段, 此时 ds 指向 TASK0 的数据段 */\
	"mov %%ax, %%es;" 					/* 初始化段寄存器指向本局部表数据段, 此时 es 指向 TASK0 的数据段 */\
	"mov %%ax, %%fs;" 					/* 初始化段寄存器指向本局部表数据段, 此时 fs 指向 TASK0 的数据段 */\
...
// 设置中断门函数(自动屏蔽随后的中断). 
// 陷阱门和中断门是调用门的特殊类, 调用门用于在不同特权级代码段之间实现受按的程序转移; 
// 中断门和陷阱门描述符有一个长指针(即描述符中包含段选择符和偏移值)，
