...
// 下面两个定义中, 宏参数前使用了前缀(unsigned), 因此 c 应该加括号, 即表示成(c). 因为在程序中 c 可能是一个复杂的表达式.
// 例如参数是 a + b, 若不加括号, 则在宏定义中变成了: (unsigned) a + b. 这显然不对. 加了括号就能正确表示成(unsigned)(a + b)
#define isascii(c) (((unsigned) (c))<=0x7f)	            // 是 ASCII 字符
#define toascii(c) (((unsigned) (c))&0x7f)	            // 转换成 ASCII 字符

// 以下两个宏定义中使用一个临时变量 _ctmp 的原因是: 在宏定义中, 宏的参数只能被使用一次. 
// 但对于多线程来说这是不安全的, 因为两个或多个线程可能在同一时刻使用这个公共临时亦是.
// 因此从 Linux 2.2.x 版本开始更改为使用两个函数来取代这从个宏定义.
