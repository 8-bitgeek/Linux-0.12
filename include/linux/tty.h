...
// 根据终端类型在 tty_table[] 中取对应终端号 nr 的 tty 结构指针. 后半部分用于根据子设备号 nr 在 tty_table[] 表中选择对应的 tty 结构.
// 如果 nr = 0, 表示正在使用前台终端, 因此直接使用终端号 fg_console 作为 tty_table[] 项索引取 tty 结构. 
// 如果 nr 大于 0, 那么就要分两种情况考虑: 1 -- nr 是虚拟终端号; 2 -- nr 是串行终端号或者伪终端号. 
// 对于虚拟终端其 tty 结构在 tty_table[] 索引项是 nr - 1(0 -- 63). 对于其他类型终端, 则它们在 tty_table 中的索引值就是 nr. 
// 例如, 如果 nr = 64, 表示是一个串行终端, 则其 tty 结构就是 tty_table[nr]. 如果 nr = 1, 则对应终端的 tty 结构是 tty_table[0].
