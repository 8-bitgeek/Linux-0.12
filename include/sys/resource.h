...
// 以下符号常数和结构用于 getrusage(). 参见 kernel/sys.c.
/*
 ...
 * 
 * 我们现在还没有支持该结构中的所有这些字段，但我们可能会支持它们的.... 否则的话，每当我们增加新的字段，
 * 那些依赖于这个结构的程序就会出问题. 现在把所有字段都包括进来就可以避免这种事情发生. 
 */
...
// Linux0.12 内核仅使用了前两个字段，它们是 timeval 结构(include/sys/time.h). 
...
// 以下是 Linux0.12 内核中所定义的资源种类, 是 getrlimit() 和 setrlimit() 中第 1 个参数 resource 的取值范围. 
// 其实这些符号常数就是进程任务结构中 rlim[] 数组的项索引值. rlim[] 数组每一项都是一个 rlimit 结构, 该结构见下面. 
