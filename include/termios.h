...
// 取相应终端 termio 结构听信息(参见 tcgetattr()). 
...
// 并且刷新(清空)输入队列, 再设置(参见 tcsetattr(), TCSAFLUSH 选项). 
#define TCSETAF		0x5408
// 等待输出队列处理完毕(空), 若参数值是 0, 则发送一个 break(参见 tcsendbreak(), tcdrain()). 
#define TCSBRK		0x5409
// 开始/停止控制. 如果参数值是 0, 则挂起输出; 如果是 1, 则重新开启挂起的输出; 
// 如果是 2, 则挂起输入; 如果是 3, 则重新开启挂起的输入(参见 tcflow()). 
#define TCXONC		0x540A
// 刷新已写输出但还没发送或已收但还没有读数据. 如果参数是 0, 则刷新(清空)输入队列; 
// 如果是 1, 则刷新输出队列; 如果是 2, 则刷新输入和输出队列(参见 tcflush()). 
...
// 读取指定终端设备的组 id, 参见 tcgetpgrp(). 该常数符号名称是 "Terminal IO Control Get PGRP" 的缩写. 
// 读取前台进程组 ID. 
...
// 对于本地连接的终端或其他设置, 软件载波标志是开启的, 对于使用 modem 线路的终端或设备则是关闭的. 
// 为了能使用这两个 ioctl 调用, tty 线路应该是以 O_NDELAY 方式打开的, 这样 open() 就不会等待载波. 
...
// 以下是控制字符数组 c_cc[] 中项的索引值. 该数组初始定义在 include/linux/tty.h 中. 
// 程序可以更改这个数组中的值. 如果定义了 _POSIX_VDISABLE(\0), 
// 那么当数组某一项值等于 _POSIX_VDISABLE 的值时, 表示禁止使用数组中相应的特殊字符.
...
#define ICRNL	0000400		// 在输入时将回车符映射成换行符 NL
...
// termios 结构中输出模式字段 c_olag 各种标志的符号常数.
...
#define ONOCR	0000020		// 在0列不输出回车符 CR
...
/* c_cflag bit meaning */	/* c_cflag 位的含义 */
...
#define  EXTB B38400		// 扩展波物率 B
#define CSIZE	0000060		// 字符位宽度屏蔽码
#define   CS5	0000000		// 每字符 5 位
#define   CS6	0000020		// 每字符 6 位
#define   CS7	0000040		// 每字符 7 位
#define   CS8	0000060		// 每字符 8 位
...
#define ECHONL	0000100		// 若设置了 ICANON, 则即使 ECHO 没有开启也回显 NL 字符
#define NOFLSH	0000200		// 当生成 SIGINT 和 SIGOUIT 信号时不刷新输入输出队列, 当生成 SIGSUSP 信号时, 刷新输入队列
#define TOSTOP	0000400		// 发送 SIGTTOU 信号到后台进程的进程组, 该后台进程试图写自己的控制终端.
#define ECHOCTL	0001000		// 若设置了 ECHO, 则除 TAB, NL, START 和 STOP 以外的 ASCII 控制信号将被回显成像 ^X 式样, X 值是控制符 +0x40
...
// 在函数库实现中, 这些函数通过调用系统调用 ioctl() 来实现. 
// 有关 ioctl() 系统调用, 请参见 fs/ioctl.c 程序. 
...
// 获取与句柄 fildes 对应对象的参数, 并将其保存在 termios_p 所指地地方. 
