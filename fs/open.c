 ...
 */
// 检查文件的访问权限. 
// 参数 filename 是文件名, mode 是检查的访问属性, 它有 3 个有效位组成: R_OK(值 4), W_OK(2), X_OK(1) 和 F_OK(0)组成, 
...
int sys_chmod(const char * filename, int mode)
{
	...
	// 该调用为指定文件设置新的访问属性 mode. 文件的访问属性在文件名对应的 i 节点中, 因此我们首先取文件名对应的 i 节点. 
	// 如果 i 节点不存在, 则返回出错码(文件或目录不存在). 
	// 如果当前进程的有效用户名 id 与文件 i 节点的用户 id 不同, 并且也不是超级用户, 则放回该文件 i 节点, 返回出错码(没有访问权限). 
	if (!(inode = namei(filename)))
		return -ENOENT;
...
int sys_open(const char * filename, int flag, int mode)
{
	...
	// 现在我们初始化打开文件的文件结构. 
	// 设置文件结构属性和标志, 置句柄引用计数为 1, 并设置 i 节点字段为打开文件的 i 节点, 初始化文件读写指针为 0. 最后返回文件句柄号.
...
int sys_close(unsigned int fd)
{
	...
	if (!(filp = current->filp[fd]))
		return -EINVAL;
	// 现在置该文件句柄的文件结构指针为 NULL. 若在关闭文件之前, 对应文件结构中的句柄引用计数已经为 0, 则说明内核出错, 停机. 
