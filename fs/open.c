...
int sys_open(const char * filename, int flag, int mode)
{
	...
	// 为了给打开文件建立一个文件句柄, 需要在进程结构中文件结构指针数组中查找一个空闲项. 
	// 空闲项的索引号 fd 即是句柄值. 若没有找到空闲项, 则返回出错码(参数无效). 			  rwx-rwx-rwx
	mode &= (0777 & ~current->umask); 				// 如果 umask = 00010010: ~(000-010-010) = 111-101-101 & 111-111-111 = 111-101-101: 即屏蔽掉成员和其他人的 w 权限.
	for (fd = 0; fd < NR_OPEN; fd++)
		if (!current->filp[fd]) 					// 文件指针为空则表示找到空闲项.
			break;
	if (fd >= NR_OPEN) 								// 没找到空闲项则返回出错码.
		return -EINVAL;
	// 然后我们设置当前进程的执行时关闭文件句柄(close_on_exec)位图, 复位对应的位. 
	// close_on_exec 是一个进程所有打开的文件句柄的位图标志.
	// 每个位(置位)代表一个打开着的文件描述符, 用于确定调用系统调用 execve() 时需要关闭的文件句柄. 
	// 当程序使用 fork() 函数创建一个子进程时, 通常会在该子进程中调用 execve() 函数加载执行另一个新程序. 
	// 此时子进程中开始执行新程序. 若一个文件句柄 close_on_exec 中的对应位被置位,
	// 那么在执行 execve() 时该对应文件句柄将被关闭, 否则该文件句柄将始终处于打开状态. 
	// 当打开一个文件时, 默认情况下文件句柄在子进程中也处于打开状态. 
	// 因此这里要复位对应位, 然后为打开的文件在系统文件表中寻找一个空闲结构项. 
