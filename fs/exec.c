...
static unsigned long * create_tables(char * p, int argc, int envc)
{
	...
	// 上面代码执行后: | 参数和环境空间剩余部分 (sp ->)| 参数指针 | NULL | 环境变量指针 | NULL | arg | env |<- 参数和环境空间末端
	...
	// 上面代码执行后: | 参数和环境空间剩余部分 (sp ->) | argc | 参数指针的指针 | 环境变量指针的指针 | 参数指针(s) | NULL | 环境变量指针(s) | NULL | arg | env | <- 参数和环境空间末端.
	// 再将参数指针和环境变量指针分别放入前面空出来的相应位置, 最后分别放置一个 NULL 指针.
	while (argc-- > 0) {
...
restart_interp:											// 脚本文件处理完后, 使用这个标号重启解释程序.
	...
	e_gid = (i & S_ISGID) ? inode->i_gid : current->egid; 		// 用于后面执行可执行文件时重新设置进程的 euid 和 eguid.
	// 如果可执行文件属于当前用户(当前进程宿主), 则把文件属性值 i 右移 6 位, 此时最低 3 位是文件宿主的访问权限标志. 
	...
	*/
	...
	// 执行下面语句之后, p 此时更改成以数据段起始处为原点的偏移值(其实就是个地址, 段基址 + p 得到线性地址), 但仍指向参数和环境空间数据开始处, 即已转换成栈指针值. 
	...
	// 进程堆起始指针字段 brk = a_text + a_data + a_bss. 
	...
	current->start_stack = p & 0xfffff000; 			// 栈指针 sp 所在页面的 4KB 边界.
	current->suid = current->euid = e_uid;
	current->sgid = current->egid = e_gid;
	// 最后将中断的调用方程序在堆栈上的代码指针 eip[0](保存的是中断返回时的下一行代码)替换为指向新程序的入口点(0x0), 
	// 并将栈指针替换为新执行文件的栈指针. 此后中断返回指令将弹出这些栈数据并使得 CPU 去执行可执行文件代码.
	// 此时新的可执行文件的内容数据还没加载到内存中, 执行时会触发缺页中断(page_fault)从磁盘中加载数据.
