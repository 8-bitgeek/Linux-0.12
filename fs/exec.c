 ...
 *
 * 我可再一次自豪地说, linux 经得起修改: 只用了不到 2 小时的工作就完全实现了需求加载处理.
 */
 ...
 */
/*
 * MAX_ARG_PAGES 定义了为新程序分配的给参数和环境变量使用的最大内存页数. 32 页内存应该足够了, 这使得环境和参数(env + arg)空间的总和达到 128KB!
 */
 ...
 *
 * 由 TYT(Tytso) 于 1991.11.24 日修改, 增加了 from_kmem 参数, 该参数指明了字符串或字符串数组是来自用户段还是内核段.
 *
 ...
 *    2          内核空间		内核空间
 *
 * 我们是通过巧妙处理 fs 段寄存器来操作的. 由于加载一个段寄存器代价太高, 所以我们尽量避免调用 set_fs(), 除非实在必要.
 */
// 复制指定个数的参数字符串到参数和环境空间中.
// 参数: argc - 欲添加的参数个数; argv - 参数指针数组; page - 参数和环境空间页面指针数组. 
// p - 参数表空间中偏移指针, 始终指向已复制串的头部; from_kmem - 字符串来源标志. 
// 在 do_execve() 函数中, p 初始化为指向参数表(128KB)空间的最后一个长字处, 参数字符串是以堆栈操作方式逆向往其中复制存放的. 
		...
		unsigned long p, int from_kmem)
{
		...
		while (len) {
			...
			if (--offset < 0) {
				...
				if (from_kmem == 2)								// 若串在内核空间则 fs 指回用户空间.
					set_fs(old_fs);
				// 如果当前偏移值 p 所在的串空间页面指针数组项 page[p/PAGE_SIZE] == 0, 表示此时 p 指针所处的空间内存页面还不存在, 
				// 则需申请一空闲内存页, 并将该页面指针填入指针数组, 同时也使页面指针 pag 指向该新页面. 若申请不到空闲页面则返回 0.
	...
	// 如果字符串和字符串数组在内核空间, 则恢复 fs 段寄存器原值. 最后, 返回参数和环境空间中已复制参数的头部偏移值.
	if (from_kmem == 2)
		set_fs(old_fs);
	return p;
}
...
static unsigned long change_ldt(unsigned long text_size, unsigned long * page)
{
	...
	// 首先把代码和数据段长度均设置为 64MB. 然后取当前进程局部描述符表代码段描述符中代码段基址. 代码段基址与数据段基址相同.
	// 再使用这些新值重新设置局部表中代码段和数据段描述符中的基址和段限长. 
	...
	char ** argv, char ** envp)
{
	...
	*(filename1 + index + 1) = '\0';
	...
	// 在正式设置执行文件的运行环境之前, 让我们先干些杂事. 内核准备了 128KB(32 个页面) 空间来存放正执行文件的命令行参数和环境字符串.
	// 上行把 p 初始设置成位于 128KB 空间的最后 1 个长字处. 在初始参数和环境空间的操作过程中, p 将用来指明在 128KB 空间中的当前位置.
	// 另外, 参数 eip[1] 是调用本次系统调用的原用户程序代码段寄存器 CS 值, 其中的段选择符当然必须是当前任务的代码段选择符(0x000f).
	// 若不是该值, 那么 CS 只能会是内核代码段的选择符 0x0008. 但这是绝对不允许的, 因为内核代码是常驻内存而不能被替换掉的. 
	// 因此下面根据 eip[1] 的值确认是否符合正常情况. 然后再初始化 128KB 的参数和环境串空间, 把所有字节清零, 并取出执行文件的 i 节点. 
...
restart_interp:
	...
	// 否则此时属性字最低 3 位就是其他用户访问该执行文件的权限. 然后我们根据属性字 i 的最低 3 位值来判断当前进程是否有权限运行这个执行文件.
	// 如果选出的相应用户没有运行该文件的权力(位 0 是执行权限), 并且其他用户也没有任何权限或者当前进程用户不是超级用户, 
	...
	// 因此从这里开始我们需要取出执行文件头部数据并根据其中的信息来分析设置运行环境, 或者运行另一个 shell 程序来执行脚本程序. 
	...
	// 在这之前我们当然需要先把函数指定的原有命令行参数和环境字符串放到 128KB 空间中, 而这里建立起来的命令行参数则放到它们前面位置处(因为是逆向放置).
	// 最后让内核执行脚本文件的解释程序. 下面就是在设置好解释程序的脚本文件名等参数后, 取出解释程序的 i 节点并跳转到 229 行执行解释程序.
	// 由于我们需要跳转到执行过的代码 229 行支. 因此在下面确认并处理了脚本文件之后需要设置一个禁止再次执行下面的脚本处理代码标志 sh_bang.
	...
	if ((bh->b_data[0] == '#') && (bh->b_data[1] == '!') && (!sh_bang)) {
		...
		// 从这里开始, 我们从脚本文件中提取解释程序名及其参数, 并把解释程序名, 解释程序的参数和脚本文件名组合放入环境参数块中. 
		// 首先复制脚本文件头 1 行字符 '#!' 后面的字符串到 buf 中, 其中含有脚本解释程序名(例如 /bin/sh), 也可能还包含解释程序的几个参数. 
		// 然后对buf中的内容进行处理. 删除开始的空格, 制表符. 
		strncpy(buf, bh->b_data + 2, 127);
		brelse(bh);             									// 释放缓冲块并放回脚本文件 i 节点. 
		iput(inode);
		buf[127] = '\0';
		if (cp = strchr(buf, '\n')) {
			*cp = '\0';     										// 第 1 个换行符换成 NULL 并去掉行头空格制表符. 
			for (cp = buf; (*cp == ' ') || (*cp == '\t'); cp++);
		}
		...
		// 此时我们得到了开头是脚本解释程序名的一行内容(字符串). 
		// 下面分析该行. 首先取第一个字符串, 它应该是解释程序名, 此时 i_name 指向该名称. 
		// 若解释程序名后还有字符, 则它们应该是解释程序的参数串, 于是令 i_arg 指向该串. 
		interp = i_name = cp;
		i_arg = 0;
		for ( ; *cp && (*cp != ' ') && (*cp != '\t'); cp++) {
 			if (*cp == '/')
		...
		// [[?? 可以看出, 实际上我们不需要去另行处理脚本文件名, 即这里完全可以复制 argc 个参数, 包括原来执行文件名(即现在的脚本文件名). 因为它位于同一个位置上]]. 
         ...
         */
		// 接着我们逆向复制脚本文件名, 解释程序的参数和解释程序文件名到参数和环境空间中. 若出错, 则置出错码, 跳转到 exec_error1. 
		// 另外, 由于本函数参数提供的脚本文件名 filename 在用户空间, 但这里赋予 copy_strings() 的脚本文件名的指针在内核空间, 
		...
		// 为了获得解释程序的 i 节点, 我们需要使用 namei() 函数, 但是该函数所使用的参数(文件名)是从用户数据空间得到的, 
		// 即从段寄存器 fs 所指空间中取得. 因此在调用 namei() 函数之前我们需要先临时让 fs 指向内核数据空间, 
	...
	/* OK, 下面开始就没有返回的地方了 */
	// 前面我们针对函数参数提供的信息对需要运行执行文件的命令行和环境空间进行了设置, 但还没有为执行文件做过什么实质性的工作, 
	// 即还没有做过为执行文件初始化进程任务结构信息, 建立页表等工作. 现在我们就来做这些工作. 由于执行文件直接使用当前进程的 "躯壳", 
	// 即当前进程将被改造成执行文件的进程, 因此我们需要首先释放当前进程占用的某些系统资源, 包括关闭指定的已打开文件, 占用的页表和内存页面等. 
	// 然后根据执行文件头结构信息修改当前进程使用的局部描述符表 LDT 中描述符的内容, 重新设置代码段和数据段描述符的限长, 
	// 再利用当前处理得到的 e_uid 和 e_gid 等信息来设置进程任务结构中相关的字段. 最后把执行本次系统调用程序的返回地址 eip[] 指向执行文件中代码的起始位置处. 
	// 这样本系统调用退出返回后就会去运行新执行文件的代码了. 注意, 虽然此时新执行文件代码和数据还没有从文件中加载到内存中, 
	// 但其参数和环境已经在 copy_strings() 中使用 get_free_page() 分配了物理内存页来保存数据, 
	// 并在 chang_ldt() 函数中使用 put_page() 到了进程逻辑空间的末端处. 
	// 另外, 在create_tables() 中也会由于在用户栈上存放参数和环境指针表而引起缺页异常, 从而内存管理程序也会就此为用户栈空间映射物理内存页.
	//
	...
	current->used_math = 0;
	// 然后我们根据新执行文件头结构中的代码长度字段 a_text 的值修改局部表中描述符基址和段限长, 并将 128KB 的参数和环境空间页面放置在数据段末端.
	// 执行下面语句之后, p 此时更改成以数据段起始处为原点的偏移值, 但仍指向参数和环境空间数据开始处, 即已转换成栈指针值. 
	// 然后调用内部函数 create_tables() 在栈空间中创建环境和参数变量指针表, 供程序的 main() 作为参数使用, 并返回该栈指针.
	p += change_ldt(ex.a_text, page);
	p -= LIBRARY_SIZE + MAX_ARG_PAGES * PAGE_SIZE;
	p = (unsigned long) create_tables((char *)p, argc, envc);
	// 接着再修改进程各字段值为新执行文件的信息. 即令进程任务结构代码尾字段 end_code 等于执行文件的代码段长度 a_text; 
	// 数据尾字段 end_data 等于执行文件的代码段长度加数据段长度(a_data + a_text); 并令进程堆结尾字段 brk = a_text + a_data + a_bss.
	...
	// 最后将原调用系统中断的程序在堆栈上的代码指针替换为指向新执行程序的入口点, 并将栈指针替换为新执行文件的栈指针. 
	// 此后返回指令将弹出这些栈数据并使得 CPU 去执行新执行文件, 因此不会返回到原调用系统中断的程序中去了.
