...
// 复制文件状态信息. 
// 参数 inode 是文件 i 节点, statbuf 是用户数据空间中 stat 文件状态结构指针, 用于存放取得的状态信息. 
...
int sys_stat(char * filename, struct stat * statbuf)
{
	...
	// 首先根据文件名找出对应的 i 节点. 然后将 i 节点上的文件状态信息复制到用户缓冲区中, 并放回 i 节点. 
...
int sys_lstat(char * filename, struct stat * statbuf)
{
	...
	// 首先根据文件名找出对应的 i 节点. 然后将 i 节点上的文件状态信息复制到用户缓冲区中, 并放回该 i 节点. 
...
int sys_fstat(unsigned int fd, struct stat * statbuf)
{
	...
	// 首先取文件句柄对应的文件结构, 然后从中得到文件的 i 节点. 然后将 i 节点上的文件状态信息复制到用户缓冲区中. 
	// 如果文件句柄值大于一个程序最多打开文件数 NR_OPEN, 或者该句柄的文件结构指针为空, 或者对应文件结构的 i 节点字段为空, 
	// 则出错, 返回出错码并退出. 
...
int sys_readlink(const char * path, char * buf, int bufsiz)
{
	...
	// 然后取得符号链接文件名的 i 节点, 并读取该文件的第 1 块数据内容. 之后放回 i 节点. 
	if (bufsiz <= 0)
		return -EBADF;
	if (bufsiz > 1023)
		bufsiz = 1023;
