...
// 变量end是由编译时的连接程序 ld 生成, 用于表明内核代码的末端, 即指明内核模块末端位置. 也可以从编译时生成的 System.map 文件中查出. 
...
// 大写名称通常都是一个宏名称, Linus 这样编写代码是为了利用这个大写名称来隐含地表示 nr_buffers 是一个在内核初始化之后不再改变的 "常量". 
// 它将在初始化函数 buffer_init() 中被设置.
int NR_BUFFERS = 0;													// 系统含有缓冲块个数.

// 等待指定缓冲块解锁.
// 如果指定的缓冲块 bh 已经上锁就让进程不可中断地睡眠在该缓冲块的等待队列 b_wait 中. 在缓冲块解锁时, 其等待队列上的所有进程将被唤醒. 
// 虽然是在关闭中断(cli)之后去睡眠的, 但这样做并不会影响在其他进程上下文中响应中断. 因为每个进程都在自己的 TSS 段中保存了标志寄存器 EFLAGS 的值, 
// 所在在进程切换时 CPU 中当前 EFLAGS 的值也随之改变. 使用 sleep_on() 进入睡眠状态的进程需要用 wake_up() 明确地唤醒.
...
int sync_dev(int dev)
{
	...
	// 因为在我们睡眠期间该缓冲块有可能已被释放或者被挪作它用, 所以在继续执行前需要再次判断一下该缓冲块是否还是指定设备的缓冲块.
...
// hash 表的主要作用是减少查找比较元素所花费的时间. 通过在元素的存储位置与关键字之间建立一个对应关系(hash 函数), 
// 我们就可以直接通过函数计算立刻查询到指定的元素. 建立 hash 函数的指导条件主要是尽量确保散列到任何数组项的概率基本相等. 
// 建立函数的方法有多种, 这里 Linux0.12 主要采用了关键字除余数法. 因为我们寻找的缓冲块有两个条件, 即设备号 dev 和缓冲块号 block, 
...
static inline void insert_into_queues(struct buffer_head * bh)
{
	...
	// 请注意当 hash 表某项第 1 次插入项时, hash() 计算值肯定为 NULL, 因此此时 hash(bh->b_dev, bh->b_blocknr) 得到的 bh->b_next 肯定是 NULL,
 ...
 */
/*
 * 代码为什么会是这样子的?我听见你问...... 原因是竞争条件. 由于我们没有对缓冲块上锁(除非我们正在读取它们的数据), 
 * 那么当我们(进程)睡眠时缓冲块可能发生一些问题(例如一个读错误将导致该缓冲块出错). 目前这种情况实际上是不会发生的, 但处理的代码已经准备好了.
 */
 ...
 */
// 从设备上读取数据块.
// 该函数根据指定的设备号 dev 和数据块号 block, 首先在高速缓冲区中申请一块缓冲块. 如果该缓冲块中经包含有有效的数据就直接返回该缓冲块指针, 
// 否则就从设备中读取指定的数据块到该缓冲块中并返回缓冲块指针.
struct buffer_head * bread(int dev, int block)
{
	...
	if (bh->b_uptodate)
		return bh;
	// 否则我们就调用底层块设备读写 ll_rw_block() 函数, 产生读设备块请求. 然后等待指定数据块被读入, 并等待缓冲区解锁. 
	// 在睡眠醒来之后, 如果该缓冲区已更新, 则返回缓冲区头指针, 退出. 否则表明读设备操作失败, 于是释放该缓冲区, 返回 NULL, 退出.
	ll_rw_block(READ, bh);
	wait_on_buffer(bh);
	if (bh->b_uptodate)
		return bh;
...
// 复制内存块.
// 从 from 地址复制一块(1024 字节)数据到 to 位置(重写的是先将 edi, esi 寄存器入栈保存起来, 拷贝完毕后再还原, 避免 edi, esi 寄存器的污染)
 ...
 */
/*
 * bread_page 一次读四个缓冲块数据读到内存指定的地址处. 它是一个完整的函数, 因为同时读取四块可以获得速度上的好处, 不用等着读一块, 再读一块了.
 */
...
struct buffer_head * breada(int dev, int first, ...)
{
	...
	if (!bh->b_uptodate)
		ll_rw_block(READ, bh);
	// 然后顺序取可变参数表中其他预读块号, 并作与上面同样处理, 但不引用. 注意, 336 行上有一个 bug. 其中的 bh 应该是 tmp. 
	// 这个 bug 直到在 0.96 版的内核代码中才被纠正过来. 另外, 因为这里是预读随后的数据块, 只需读进调整缓冲区但并不马上就使用, 
...
// 缓冲区初始化函数
// 参数 buffer_end 是缓冲区内存末端. 对于具有 16M 内存的系统, 缓冲区末端被设置为 4MB. 对于有 8MB 内存的系统, 缓冲区末端被设置 2MB. 
...
void buffer_init(long buffer_end)
{
	...
	// 所以实际可用缓冲区内存高端位置应该是 640KB. 否则缓冲区内存高端一定大于 1MB.
	if (buffer_end == 1 << 20)
		b = (void *) (640 * 1024);
	else
		b = (void *) buffer_end;
