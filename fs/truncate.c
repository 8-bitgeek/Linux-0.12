...
// 释放所有一次间接块. (内部函数)
// 参数 dev 是文件系统所有设备的设备号; block 是逻辑块号. 成功则返回 1, 否则返回 0. 
static int free_ind(int dev, int block) {
	...
	// 首先判断参数的有效性. 如果逻辑块号为 0, 则返回. 然后读取一次间接块, 并释放其上表明使用的所有逻辑块, 
	// 然后释放该一次间接块的缓冲块. 函数 free_block() 用于释放设备上指定逻辑块号的磁盘块(fs/bitmap.c). 
	if (!block) {
		return 1;
	}
	block_busy = 0;
...
static int free_dind(int dev, int block) {
	...
	// 首先判断参数的有效性. 如果逻辑块号为 0, 则返回. 
	// 然后读取二次间接块的一级块, 并释放其上表明使用的所有逻辑块, 然后释放该一级块的缓冲块. 
	if (!block) {
		return 1;
	}
	block_busy = 0;
...
void truncate(struct m_inode * inode) {
	...
	// 然后释放 i 节点的 7 个直接逻辑块, 并将这 7 个逻辑块项全置零. 
	// 函数 free_block() 用于释放设备上指定逻辑块的磁盘块(fs/bitmap.c). 
	// 若有逻辑块忙而没有被释放则置块忙标志 block_busy. 
repeat:
	block_busy = 0;
	for (i = 0; i < 7; i++) {
	...
	// 此后设置 i 节点已修改标志, 并且如果还有逻辑块由于 "忙" 而没有被释放, 则把当前进程运行时间片置 0, 
	// 以让当前进程先被切换去运行其他进程, 稍等一会再重新执行释放操作. 
	inode->i_dirt = 1;
	if (block_busy) {
	...
	// 最后重新置文件修改时间和 i 节点改变时间为当前时间. 宏 CURRENT_TIME 定义在头文件 include/linux/sched.h 中, 
	// 定义为(startup_time + jiffies/HZ). 用于取得从 1970:0:0:0 开始到现在为止经过的秒数. 
