...
int sys_mount(char * dev_name, char * dir_name, int rw_flag)
{
	...
	// 现在安装点也检查完毕, 我们开始读取要安装文件系统的超级块信息. 如果读超级块操作失败, 
	// 则放回该安装点 i 节点 dir_i 并返回出错码. 一个文件系统的超级块会首先从超级块表中进行搜索, 如果不在超级块表中就从设备上读取.
	if (!(sb = read_super(dev))) {
		iput(dir_i);
		return -EBUSY;
	}
	// 在等到了文件系统超级块之后, 我们对它先进行检测一番. 如果将要被安装的文件系统已经安装在其他地方, 则放回该 i 节点, 返回出错码. 
...
// 安装根文件系统.
// 该函数属于系统初始化操作的一部分. 函数首先初始化文件表数组 file_table[] 和超级块表(数组), 然后读取根文件系统超级块, 
// 并取得文件系统根 i 节点. 最后统计并显示出根文件系统上的可用资源(空闲块数和空闲 i 节点数 0. 
...
void mount_root(void)
{
	...
	// 做好以上 "份外" 的初始化工作之后, 我们开始安装根文件系统. 于是从根设备上读取文件系统超级块, 并取得文件系统的根 i 节点(1 号节点)在内存 i 节点表中的指针.
	...
	// 在显示过设备上空闲逻辑块数/逻辑块总数之后. 我们再统计设备上空闲 i 节点数. 首先令 i 等于超级块中表明的设备上 i 中总数 + 1. 
