 ...
 */
/*
 * Ok, 需求加载是比较容易编写的, 而共享页面却需要有点技巧. 共享页面程序是 91.12.2 开始编写的, 好像能够工作 - Linus. 
 *
 * 通过执行大约 30 个 /bin/sh 对共享操作进行了测试: 在老内核当中需要占用多于 6MB 的内存, 而目前却不用. 
 * 现在看来工作得很好. 
 *
 ...
 */
/*
 * 91.12.18 开始编写真正的虚拟内存管理 VB(交换页面到/从磁盘). 需要对此考虑很多并且需要作很多工作. 呵呵, 也只能这样了. 
...
{
	// 首先判断参数给定的物理地址 addr 的合理性. 如果物理地址 addr 小于内存低端(1MB), 则表示在内核程序或高速缓冲中, 对此不予处理.
...
// 页目录位于物理地址 0 开始处, 共 1024 项, 每项 4 字节, 共占 4KB. 每个目录项指定一个页表. 内核页表物理地址 0x1000 处开始(紧接着目录空间),
// 共 4 个页表. 每个页表有 1024 项, 每项 4B. 因此也占 4KB(1 页)内存. 
 ...
 */
// 复制目录表项和页表项.
// 复制指定线性地址和长度内存对应的页目录项和页表项, 从而被复制的页目录和页表对应的原物理内存页面区被两套页表映射而共享使用. 
...
int copy_page_tables(unsigned long from, unsigned long to, long size)
{
	...
	// 作这样的要求是因为一个页表的 1024 项可管理 4MB 内存. 源地址 from 和目的地址 to 只有满足这个要求才能保证从一个页表的第 1 项开始复制页表项, 
	// 并且新页表最初所有项都是有效的. 然后取得源地址和目的地址的起始目录项指针(from_dir 和 do_dir). 
	...
	for( ; size-- > 0 ; from_dir++, to_dir++) {
		...
		*to_dir = ((unsigned long) to_page_table) | 7;
		// 然后针对当前处理的页目录项对应的页表, 设置需要复制的页面项数. 如果是在内核空间, 则仅需复制头 160 页对应的页表项(nr = 160),
		// 对应于开始 640KB 物理内存. 否则需要复制一个页表中的所有 1024 个页表项(nr = 1024), 可映射 4MB 物理内存.
		nr = (from == 0) ? 0xA0 : 1024;
		// 此时对于当前页表, 开始循环复制指定的 nr 个内存页面表项. 先取出源页表项内容, 如果当前源页面没有使用(项内容为 0), 则不用复制该表项, 继续处理下一项.
		for ( ; nr-- > 0 ; from_page_table++, to_page_table++) {
			...
			// 复位页表项中 R/W 标志(位 1 置 0), 即让页表项对应的内存页面只读, 然后将该页表项复制到目的页表中
			this_page &= ~2;
			*to_page_table = this_page;
			...
			// 157 行语句含义是令源页表项所指内存页也为只读. 因为现在开始已有两个进程共用内存区了. 
...
// 或者说是把线性地址空间中指定地址 address 处的页面映射到主内存区页面 page 上. 主要工作是在相关页目录项和页表项中设置指定页面的信息. 若成功则返回物理页面地址.
...
void un_wp_page(unsigned long * table_entry)
{
	...
	// 首先取参数指定的页表项中物理页面位置(地址)并判断该页面是不是共享页面. 如果原页面地址大于内存低端 LOW_MEM(表示在主内存区中), 
	// 并且其在页面映射字节图数组中值为 1(表示页面仅被引用 1 次, 页面没有被共享), 则在该页面的页表项中 R/W 标志(可写), 并刷新页变换高速缓冲, 然后返回. 
	...
	// 否则就需要在主内存区内申请一页空闲页面给执行写操作的进程单独使用, 取消页面共享. 如果原页面大于内存低端(则意味着 mem_map[] > 1, 页面是共享的),
	// 则将原页面的页面映射字节数组值递减 1. 然后将指定页表项内容更新为新页面地址, 并置可读写标志(U/S, R/W, P). 在刷新页变换高速缓冲之后, 最后将原页面内容复制到新页面.
	if (!(new_page = get_free_page()))
		oom();											// 内存不够处理.
	if (old_page >= LOW_MEM)
...
void do_wp_page(unsigned long error_code, unsigned long address)
{
	...
	// 而 *((address >> 20) & 0xffc) 则是取指定目录表项内容中对应页表的物理地址. 最后与上 0xffffff000 用于屏蔽掉页目录项内容中的一些标志位(目录项低 12 位).
...
// 取得一页空闲内存并映射到指定线性地址处.
// get_free_page() 仅是申请取得了主内存区的一页物理内存. 而本函数则不仅是获取到一页物理内存页面, 还进一步调用 put_page(), 将物理页面映射到指定的线性地址处.
...
void get_empty_page(unsigned long address)
{
	...
	// 292 行上英文注释的含义是: free_page() 函数的参数 tmp 是0也没有关系, 该函数会忽略它并能正常返回.
...
// 当前进程与进程 p 是同一执行代码, 也可以认为当前进程是由 p 进程执行 fork 操作产生的进程, 因此它们的代码内容一样. 
// 如果未对数据段内容作过修改那么数据段内容也应一样. 参数 address 是进程中的逻辑地址, 即是当前进程欲与 p 进程共享页面的逻辑页面地址. 
// 进程 p 是将被共享页面的进程. 如果 p 进程 address 处的页面存在并且没有被修改过的话, 就让当前进程与 p 进程共享之. 
...
static int try_to_share(unsigned long address, struct task_struct * p)
{
	...
	// 首先分别求得指定进程 p 中和当前进程中逻辑地址 address 对应的页目录项. 为了计算方便先求出指定逻辑地址 address 处的 “逻辑” 页目录号, 
	// 即以进程空间(0 - 64MB)算出的页目录项号. 该 “逻辑” 页目录项号加上进程 p 在 CPU 4GB 线性空间中起始地址对应的页目录项, 
	...
	// 在得到 p 进程和当前进程 address 对应的目录项后, 下面分别对进程 p 和当前进程处理. 首先对p进程的表项进行操作. 
	// 目录是取得 p 进程中 address 对应的物理内在页面地址, 并且该物理页面存在, 而且干净(没有被修改过, 不脏). 
	// 方法是先取目录项内容. 如果该目录项元效(P = 0), 表示目录项对应的二级页表不存在, 于是返回. 
	// 否则取该目录项对应页表地址 from, 从而计算出逻辑地址 address 对应的页表项指针, 并取出该面表项内容临时保存在 phys_addr 中. 
	...
	// 接着看看页表项映射的物理页面是否存在并且干净. 0x41 对应页表项中的 D(Dirty) 和 P(present) 标志. 如果页面不干净或无效则返回. 
	// 然后我们从该表项中取出物理页面地址再保存在 phys_addr 中. 
	// 最后我们再检查一下这个物理页面地址的有效性, 即它不应该超过机器最大物理地址值, 也不应该小于内在低端(1MB). 
	...
	if (phys_addr >= HIGH_MEMORY || phys_addr < LOW_MEM)
		return 0;
	// 下面首先对当前进程的表项进行操作. 目标是取得当前进程中 address 对应的页表项地址, 并且该页表项还没有映射物理页面, 即其 P = 0. 
	// 首先取当前进程页目录项内容 -> to. 如果该目录项元效(P = 0), 即目录项对应的二级页表不存在, 则申请一空闲页面来存放页表, 
	...
	if (!(to & 1))
		if (to = get_free_page())
			*(unsigned long *) to_page = to | 7;
			...
			oom();
	// 否则取目录项中的页表地址 -> to, 加上页表索引值 << 2, 即页表项在表中偏移地址, 得到页表项地址 -> to_page. 
	// 针对该页表项, 如果此时我们检查出其对应的物理页面已经存在, 即页表项的存在位 P = 1, 则说明原本我们想共享进程 p 中对应的物理页面, 
	...
	*(unsigned long *) to_page = *(unsigned long *) from_page;
	// 随后刷新页变换高速缓冲. 计算所操作物理页面的页面号, 并将对应页面映射字节数组项中的引用递增 1. 最后返回 1, 表示共享处理成功. 
	invalidate();
	phys_addr -= LOW_MEM;
	phys_addr >>= 12;                       						// 得页面号. 
	mem_map[phys_addr]++;
 ...
 */
/*
 * share_page() 试图找到一个进程, 它可以与当前进程共享页面. 参数 address 是当前进程数据空间中期望共享的某页面地址.
 *
 * 首先我们通过检测 executable -> i_count 来查证是否可行. 如果有其他任务已共享该 inode, 则它应该大于 1.
 */
// 共享页面处理.
...
// 判断系统中是否有另一个进程也在执行同一个执行文件的方法是利用进程任务数据结构中的 executable 字段(或 library 字段). 
// 该字段指向进程正在执行程序(或使用的库文件)在内存中的 i 节点. 根据该 i 节点的引用次数 i_count 我们可以进行这种判断. 
// 若节点的 i_count 值大于 1, 则表明系统中有两个进程正在运行同一个执行文件(或库文件), 
// 于是可以再对任务结构数组中所有任务比较是否有相同的 executable 字段(或 library 字段)来最后确定多个进程运行着相同执行文件的情况.
// 参数 inode 是欲进行共享页面进程执行文件的内存 i 节点. address 是进程中的逻辑地址, 即当前进程欲与 p 进程共享页面的逻辑页面地址. 
// 返回 1 - 共享操作成功, 0 - 失败.
static int share_page(struct m_inode * inode, unsigned long address)
{
	...
	if (inode->i_count < 2 || !inode)
		return 0;
	// 否则搜索任务数组中所有任务. 寻找与当前进程可共享页面的进程, 即运行相同执行文件的另一个进程, 并尝试对指定地址的页面进行共享. 
	// 若进程逻辑地址 address 小于进程库文件在逻辑地址空间的起始地址 LIBRARY_OFFSET, 则表明共享的页面在进程执行文件对应的逻辑地址空间范围内, 
	// 于是检查一下指定 i 节点是否与进程的执行文件 i 节点(即进程 executable 相同, 若不相同则继续寻找. 
	// 若进程逻辑地址 address 大于等于进程库文件在逻辑地址空间的起始地址 LIBRARY_OFFSET, 则表明想要共享的页面在进程使用的库文件中, 
	// 于是检查指定节点 inode 是否与进程的库文件i节点相同, 若不相同则继续寻找. 
	// 如果找到某个进程 p, 其 executable 或 library 与指定的节点 inode 相同, 则调用页面试探函数 try_to_share() 尝试页面共享. 
...
// error_code 指出出错类型; address 产生异常的页面线性地址.
// 该函数首先查看所缺页是否在交换设备中, 若是则交换进来. 否则尝试与已加载的相同文件进行页面共享, 
// 或者只是由于进程动态申请内存页面而只需映射一页物理内存页即可. 若共享操作不成功, 那么只能从相应文件中读入所缺的数据页面到指定线性地址处.
void do_no_page(unsigned long error_code, unsigned long address)
{
	...
	// 然后根据指定的线性地址 address 求出其对应的二级页表项指针, 并根据该页表项内容判断 address 处的页面是否在交换设备中. 若是则调入页面并退出.
	// 方法是首先取指定线性地址 address 对应的目录项内容. 如果对应的二级页表存在, 则取出该目录项中二级页表的地址, 
	// 加上页表项偏移值即得到线性地址 address 处页面对应的页表项指针, 从而获得页表项内容. 
	// 若页表内容不为 0 并且页表项存在位 P = 0, 则说明该页表项指定的物理页面应该在交换设备中. 于是从交换设备中调入指定页面后退出函数.
	...
	// 否则取线性空间中指定地址 address 处页面地址, 并算出指定线性地址在进程空间中相对于进程基址的偏移长度值 tmp, 即对应的逻辑地址. 
...
// 物理内存管理初始化.
// 该函数对 1MB 以上内存区域以页面为单位进行管理前的初始化设置工作. 一个页面长度为 4KB 字节. 
// 该函数将 1MB 以上所有物理内存划分成一个个页面, 并使用一个页面映射字节数组 mem_map[] 来管理所有这些页面. 
...
// 显示系统内存信息.
// 根据内存映射字节数组 mem_map[] 中的信息以及页目录和页表内容统计系统中使用的内存页面数和主内存区中总物理内存页面数. 
...
void show_mem(void)
{
	...
	// 根据内存映射字节数组 mem_map[], 统计系统主内存区页面总数 total, 以及其中空闲页面数 free 和被共享的页面数 shared. 并显示这些信息.
	printk("Mem-info:\n\r");
	for(i = 0 ; i < PAGING_PAGES ; i++) {
	...
	// 统计处理器分页管理逻辑页面数. 页目录表前 4 项供内核代码使用, 不列为统计范围, 因此扫描处理的页目录项从第 5 项开始. 
	// 方法是循环处理所有页目录项(除前 4 个项), 若对应的二级页表存在, 那么先统计二级页表本身占用的内存页面, 然后对该页表中所有页表项对应页面情况进行统计.
	k = 0;													// 一个进程占用页面统计值.
	for(i = 4 ; i < 1024 ;) {
		if (1 & pg_dir[i]) {
			// (如果页目录项对应二级页表地址大于机器最高物理内存地址 HIGH_MEMORY, 说明该目录项有问题. 于是显示该目录项信息并继续处理下一个目录项.
			if (pg_dir[i] > HIGH_MEMORY) {					// 目录项内容不正常.
				printk("page directory[%d]: %08X\n\r",
					i, pg_dir[i]);
				continue;
			}
			// 如果页目录项对应二级页表的 "地址" 大于 LOW_MEM(即 1MB), 则把一个进程占用的物理内存页统计值 k 增 1, 把系统占用的所有物理内存页统计值 free 增 1.
			// 然后邓对应页表地址 pg_tb1, 并对该页表中所有页表项进行统计. 
			// 如果当前页表项所指物理页面存在并且该物理页面 "地址" 大于 LOW_MEME, 那么就将页表项对应页面纳入统计值.
			if (pg_dir[i] > LOW_MEM)
				free++, k++;								// 统计页表占用页面.
			pg_tbl = (unsigned long *) (0xfffff000 & pg_dir[i]);
			for(j = 0 ; j < 1024 ; j++)
				if ((pg_tbl[j]&1) && pg_tbl[j] > LOW_MEM)
					// (若该物理页面地址大于机器最高物理内存地址 HIGH_MEMORY, 则说明该页表项内容有问题, 于是显示该页表项内容. 否则将页表项对应页面纳入统计值.)
		...
		// 因每个任务线性空间长度是 64MB, 所以一个任务占用 16 个目录项. 因此这每统计了 16 个目录项就把进程的任务结构占用的页表统计进来. 
