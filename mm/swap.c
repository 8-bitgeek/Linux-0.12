 ...
 */
/*
 * 获取首个(实际上是最后 1 个)空闲物理内存页面, 并标志为已使用. 如果没有空闲页面则返回 0.
 */
// 只有内核能调用该函数, 所以是由内核统一管理主内存区的内存分配. mem_map 也只有内核代码可以访问.
...
// 输出: 返回 %0(ax = 空闲页面起始物理地址, 即真实的物理内存地址). 
