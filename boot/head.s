...
1:	incl %eax							# check that A20 really IS enabled
	 ...
	 # 为了引用一个局部标号的下一个定义, 而要与成 'Nf', 这里 N 是 10 个前向引用之一. 
	 # 上面 'b' 表示 "向后(backwards)", 'f' 表示 "向前(forwards)". 在汇编程序的某一处, 我们最大可以向后/向前引用 10 个标号.
 ...
 */
/*
 * 注意! 在下面这段程序中, 486 应该将们 16 置位, 以检查在超级用户模式下的写保护, 此后 "verify_area()" 调用就不需要了. 
...
tmp_floppy_area:
 ...
 # 当 head.s 最后执行 ret 指令时就会弹出 main() 的地址, 并把控制权转移到 init/main.c 程序中.
 ...
 */
/*
 * 这个子程序通过设置控制寄存器 cr0 的标志(PG 位 31)来启动对内存的分页处理功能, 并设置各个页表项的内容, 以恒等映射前 16MB 的物理内存. 
 * 分页器假定不会产生非法的地址映射(也即在只有 4MB 的机器上设置出大于 4MB 的内存地址).
 *
 * 注意! 尽管所有的物理地址都应该由这个子程序进行恒等映射, 但只有内核页面管理函数能直接使用 > 1MB 的地址. 
 * 所有 "普通" 函数仅使用低于1MB的地址空间, 或者是使用局部数据空间, 该地址空间将被映射到其他一些地方去 -- mm(内存管理程序)会管理这些事的.
 *
 */
 # 上面英文注释第 2 段的含义是指在机器物理内存中大于 1MB 的内存空间主要被用于主内存区. 主内存区空间由 mm 模块管理.
 # 它涉及页面映射操作. 内核中所有其它函数就是这里指的一般(普通)函数. 若要使用主内存区的页面, 就需要使用 get_free_page() 等函数获取. 
...
setup_paging:							# 首先对 5 页内存(1 页目录 + 4 页页表)清零.
	...
	movl $pg3 + 7, pg_dir + 12			/*  --------- " " --------- */
	 ...
	 # 下面 6 行填写 4 个页表中所有项的内容, 共有: 4(页表) * 1024(项/页表) = 4096 项(0 - 0xfff), 即能映射物理内存 4096 * 4KB = 16MB.
	 # 每项的内容是: 当前项所映射的物理内存地址 + 该页的标志(这里均为 7).
	 # 使用的方法是从最后一个页表的最后一项开始按倒退顺序填写. 一个页表的最后一项在页表中的位置是 1023 * 4 = 4092. 
	 # 因此最后一页的最后一项的位置就是 $pg3 + 4092.

	movl $pg3 + 4092, %edi				# edi -> 最后一页的最后一项.
	movl $0xfff007, %eax				/*  16Mb - 4096 + 7 (r/w user,p) */
										# 最后一项对应物理内存页的地址是 0xfff000, 加上属性标志 7, 即为 xfff007.
	std									# 方向位置位, edi 值递减(4 字节).
1:	stosl								/* fill pages backwards - more efficient :-) */
	subl $0x1000, %eax					# 每填好一项, 物理地址值减 0x1000.
	jge 1b								# 如果小于 0 则说明全填写好了.
	cld
