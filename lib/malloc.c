 ...
 *
 * 限制: 使用该函数一次所能分配的最大内存是 4KB, 即 Linux 中内存页面的大小. 
 *
 * 编写该函数所遵循的一般规则是每页(被称为一个存储桶)仅分配所要容纳对象的大小. 
 * 当一页上的所有对象都释放后, 该页就可以返回通用空闲内存池. 
 * 当 malloc() 被调用时, 它会寻找满足要求的最小的存储桶, 并从该存储桶中分配一块内存. 
 *
 * 每个存储桶都有一个作为其控制用的存储描述符, 其中记录了页面上有多少个对象正被使用以及该页上空闲内存的列表. 
 * 就像存储桶自身一样, 存储桶描述符也是存储在使用 get_free_page() 申请到的页面上的, 但是与存储桶不同的是, 
 * 桶描述符所占用的页面将不再会释放给系统. 幸运的是一个系统大约只需要 1 到 2 页的桶描述符页面, 
 * 因为一个页面可以存放 256 个桶描述符(对应 1MB 内存的存储页面). 
 * 如果系统为桶描述符分配了许多内存, 那么肯定系统什么地方出了问题. 
 *
 * 注意! malloc() 和 free() 两者关闭了中断的代码部分都调用了 get_free_page() 和 free_page() 函数, 
 * 以使 malloc() 和 free() 可以安全地被从中断程序中调用(当网络代码, 尤其是 NFS 等被加入到 Linux 中时就可能需要这种功能). 
 * 但前提是假设 get_free_page() 和 free_page() 是可以安全地在中断级程序中使用的, 这在一旦加入了分页处理之后就可能不是安全的. 
 * 如果真是这种情况, 那么我们就需要修改 malloc() 来 “预先分配” 几页不用的内存, 
 * 如果 malloc() 和 free() 被从中断程序中调用时就可以安全地使用这些页面. 
 *
 * 另外需要考虑到的是 get_free_page() 不应该睡眠: 如果会睡眠的话, 则为了防止任何竞争条件, 代码需要仔细地安排顺序. 
 ...
 *
 * 如果 Linux 内核分配了许多指定大小的对象, 那么我们就希望将该指定的大小加到该列表(链表)中, 因为这样可以使内存的分配更有效. 
...
static inline void init_bucket_desc()
{
	 ...
	 */
    /*
     * 这是在最后处理的, 目的是为了避免在 get_free_page() 睡眠时该子程序又被调用而引起的竞争条件. 
     */
...
void * malloc(unsigned int len)
{
	...
	if (!bdesc) {
		...
		// 初始化该新的桶描述符. 令其引用数量等于 0; 桶的大小等于对应桶目录的大小; 申请一内在页面, 让描述符的页面指针 page 指向该页面; 
        ...
        /* 在该页空闲内存中建立空闲对象链表 */
		// 以该桶目录项指定的桶大小为对象长度, 对该页内存进行划分, 并使每个对象的开始 4 字节设置成指向下一对象的指针. 
		for (i = PAGE_SIZE / bdir->size; i > 1; i--) {
			*((char **) cp) = cp + bdir->size;
			cp += bdir->size;
		}
		// 最后一个对象开始处的指针设置为 0(NULL). 
		// 然后让该桶描述符的下一描述符指针字段指向对应桶目录项指针 chain 所指的描述符, 而桶目录的 chain 指向该桶描述符, 
		// 即将该描述符插入到描述符链链头处. 
		*((char **) cp) = 0;
 ...
 */
/*
 * 下面是释放子程序. 如果你知道释放对象的大小, 则 free_s() 将使用该信息加快搜寻对应桶描述符的速度. 
 *
...
void free_s(void *obj, int size)
{
	...
	for (bdir = bucket_dir; bdir->size; bdir++) {
		...
		if (bdir->size < size)
			continue;
		// 搜索对应目录项中链接的所有描述符, 查找对应页面. 如果某描述符页面指针等于 page 则表示找到了相应的描述符, 跳转到found. 
		// 如果描述符不含有对应 page, 则让描述符指针 perv 指向该描述. 
		for (bdesc = bdir->chain; bdesc; bdesc = bdesc->next) {
			if (bdesc->page == page)
				goto found;
			prev = bdesc;
...
found:
	...
	*((void **) obj) = bdesc->freeptr;
	...
	if (bdesc->refcnt == 0) {
		...
		// 如果 prev == NULL, 则说明当前一个描述符是该目录项第 1 个描述符, 即目录项中 chain 应该直接指向当前描述符 bdesc, 
		// 否则表示链表有问题, 则显示出错信息, 死机. 因此, 为了将当前描述符从链表中删除, 应该让 chain 指向下一个描述符. 
		else {
			if (bdir->chain != bdesc)
