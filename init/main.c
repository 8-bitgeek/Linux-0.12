...
// 定义宏. 将 BCD 码转换成二进制值. BCD 码利用半个字节(4 位)表示一个 10 进制数, 因此一个字节表示 2 个 10 进制数. (val)&15 取 BCD
// 表示的 10 进制个位数, 而 (val)>>4 取 BCD 表示的 10 进制十位数, 再乘以 10. 因此最后两者相加就是一个字节 BCD 码的实际二进制数值.
#define BCD_TO_BIN(val) ((val) = ((val)&15) + ((val) >> 4) * 10)

// 该函数取 CMOS 实时钟信息作为开机时间, 并保存到全局变量 startup_time(秒) 中. 
// 其中调用的函数 kernel_mktime() 用于计算从 1970 年 1 月 1 日 0 时起到开机当日经过的秒数, 作为开机时间.
