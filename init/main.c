...
// 下面的 _syscall0() 是 unistd.h中的内嵌宏代码. 以嵌入汇编的形式调用 Linux 的系统调用中断 0x80. 该中断是所有系统调用的入口.
...
void init(void)
{
	...
	// 这三句的作用是父进程等待子进程的结束. &i 是存放返回状态信息的位置. 如果 wait() 返回值不等于子进程号, 则继续等待.
  	if (pid > 0)
		while (pid != wait(&i));
	// 如果执行到这里, 说明刚创建的子进程的执行已停止或终止了. 下面循环中首先再创建一个子进程, 如果出错, 则显示 “初始化程序创建子进程失败” 信息并继续执行. 
