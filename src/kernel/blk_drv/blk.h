 ...
 * 注意, 写操作仅使用这些项中末尾的 2/3 项; 读操作优先处理.
 *
...
extern struct request request[NR_REQUEST];              // 请求队列数组, 共 32 项.
extern struct task_struct * wait_for_request;           // 等待空闲请求项的进程队列头指针.

// 设备数据块总数指针数组. 每个指针项指向指定主设备号的总块数组 hd_sizes[]. 
...
// 如果定义了设备中断处理符号常数, 则把它声明为一个函数指针, 默认为 NULL.
#ifdef DEVICE_INTR
...
#endif
// 声明设备请求符号常数 DEVICE_REGUEST 是一个不带参数并无返回的静态函数指针.
...
// 否则, 如果当前请求项中的主设备号不等于驱动程序定义的主设备号, 说明请求项队列乱掉了, 于是内核显示出错信息并停机. 
// 否则若请求中用的缓冲块没有被锁定, 也说明内核程序出了问题, 于是显示出错信息并停机.
#define INIT_REQUEST \
repeat: \
	if (!CURRENT) {											/* 如果当前请求项指针为 NULL 则返回 */\
		CLEAR_DEVICE_INTR \
