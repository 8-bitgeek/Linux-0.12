...
long kernel_mktime(struct tm * tm) {
	...
	// 首先计算 1970 年到现在经过的年数. 因为是 2 位表示方式, 所以会有 2000 年问题. 我们可以简单地在最前面添加一条语句来解决这个问题:
	// if(tm->tm_year<70) tm->tm_year += 100; 由于 UNIX 计年份 y 是从 1970 年算起. 到 1972 年就是一个闰年, 因此过 3 年(71,72,73)
	// 就是第 1 个闰年, 这样从 1970 年开始的闰年数计算方法就应该是 1+(y-3)/4, 即为 (y+1)/4. 
