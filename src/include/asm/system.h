...
// 再通过调用 iret 实现中断返回, 返回(切换)到任务 0 上执行.]
// 中断调用时会切换到新栈(堆栈切换), 并依次入栈: ss, esp, eflags, cs, eip.
// 中断返回时会依次从栈中加载原: eip(栈顶), cs, eflags, esp, ss(栈底) 这几个寄存器的值.
// 先构造任务 0 调用中断时压栈的场景(task0 CPL = 3, 所以要模拟t特权级发生变化时的中断调用过程)，再利用 iret 来切换到任务 0.
...
__asm__ (\
	...
	"pushl $1f;"  						/* 将下面标号 1 的偏移地址(eip)入栈. 即中断返回后以 task0 从下面 1 处继续运行 */\
	"iret;"  							/* 执行中断返回指令(此处只是实现了特权级切换, 而不是真正的任务切换) */\
...
__asm__ (\
	"movw $104, %1;"							/* 将 TSS(或 LDT)长度放入描述符长度域(第 0-1 字节) */\
	"movw %%ax, %2;"							/* 将基地址的低字放入描述符第 2-3 字节 */\
	"rorl $16, %%eax;"							/* 将基地址高字右循环移入 ax 中(低字则进入高字处) */\
	"movb %%al, %3;"  							/* 将基地址高中低字节移入描述符第 4 字节 */\
	"movb $" type ", %4;"  						/* 将类型标志字节(DPL/TYPE 等)移入描述符第 5(从 0 开始) 字节 */\
	"movb $0x00, %5;"  							/* 描述符第 6 字节置 0 */\
	"movb %%ah, %6;"  							/* 将基地址高字中高字节移入描述符第 7 字节 */\
