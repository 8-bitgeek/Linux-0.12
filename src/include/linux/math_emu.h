...
struct info {
	...
	long ___edi;                    // 异常中断 int 7 处理过程入栈的寄存器。
	long ___esi;
	long ___ebp;
 ...
 */
/*
 * GCC 强制对齐问题是愚蠢: 我想仅仅使用两个 long 去表示临时实型64位的有效数, 但.
 */
// 临时实数结构
typedef struct {
	long a,b;               // 有效数. 
...
struct swd {
	...
	int c1:1;
	int c2:1;
	int top:3;              // 用于指明当前哪个 80 位寄存器位于栈顶. 
	int c3:1;
	int b:1;                // 忙. 
};
...
#define CONSTZ   (temp_real_unaligned) {0x0000,0x0000,0x0000,0x0000,0x0000}     // 常数 0.0. 
#define CONST1   (temp_real_unaligned) {0x0000,0x0000,0x0000,0x8000,0x3FFF}     // 临时实数 1.0. 
#define CONSTPI  (temp_real_unaligned) {0xC235,0x2168,0xDAA2,0xC90F,0x4000}     // 常数 Pi. 
#define CONSTLN2 (temp_real_unaligned) {0x79AC,0xD1CF,0x17F7,0xB172,0x3FFE}     // 常数 Loge(2). 
#define CONSTLG2 (temp_real_unaligned) {0xF799,0xFBCF,0x9A84,0x9A20,0x3FFD}     // 常数 Log10(2). 
#define CONSTL2E (temp_real_unaligned) {0xF0BC,0x5C17,0x3B29,0xB8AA,0x3FFF}     // 常数 Log2(e). 
#define CONSTL2T (temp_real_unaligned) {0x8AFE,0xCD1B,0x784B,0xD49A,0x4000}     // 常数 Log2(10). 
...
// 下面宏定义用于设置状态字中条件标志位 C0, C1, C2 和 C3. 
...
void fcom(const temp_real *, const temp_real *);        // 仿真浮点指令 FTST. 
