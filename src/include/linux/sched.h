...
#define TASK_UNINTERRUPTIBLE	2	// 进程处于不可中断等待状态, 主要用于 I/O 操作等待. 处于该状态下的进程不能被任何信号中断, 包括 KILL.
...
struct task_struct {
	...
	int tty;							// 进程使用 tty 终端的子设备号. -1 表示没有使用.
	unsigned short umask;				// 文件创建属性屏蔽码(哪一位置位就表示要屏蔽哪一位).
	struct m_inode * pwd;				// 当前工作目录 i 节点结构指针.
	struct m_inode * root;				// 根目录 i 节点结构指针.
...
/* close_on_exec 是一个进程所有文件句柄的位图标志. 
   每个位代表一个打开着的文件描述符, 用于确定在调用系统调用 execve() 时需要关闭的文件句柄. 
   当程序使用 fork() 函数创建一个子进程时, 通常会在该子进程中调用 execve() 函数加载执行另一个新程序. 
   此时子进程中开始执行新程序. 若一个文件句柄在 close_on_exec 中的对应位被置位, 
   那么在执行 execve() 时该对应文件句柄将被关闭, 否则该文件句柄将始终处于打开状态. 
   参见 fs/exec.c do_execve() 对 close_on_exec 的处理.
*/
...
// 因此 __tmp 中 a 的值是 32 位偏移值, 而低 2 字节是新 TSS 段的选择符(高 2 字节不用). 
...
// 是通过将当前任务指针与保存在 last_task_used_math 变更中的上次使用过协处理器指针进行比较而作出的, 
// 参见文件 kernel/sched.c 中有关 math_state_restore() 函数的说明. 
// 该宏的唯一的调用方是 schedule() 方法, 也即系统启动后只有 schedule 会进行任务切换.
#define switch_to(n) { \
...
// 指令 lsll 是 Load Segment Limit 的缩写, 它从指定段描述符中取出分散的限长比特位拼成完整的段限长值放入指定寄存器中. 
