 ...
 */
/*
 * OK, Peter 编制了复杂但很直观的多个 _wait() 函数. 
 * 我对这些函数进行了改写, 以使之更简洁: 这些代码可能不容易看懂, 但是其中应该不会存在竞争条件问题, 并且很实际. 
 * 如果你能理解这里编制的代码, 那么就说明你已经理解 Linux 中睡眠/唤醒的工作机制. 
 *
 * 两个很简单的过程, add_wait() 和 free_wait() 执行了主要操作. 在整个 select 处理过程中我们不得不禁止中断. 
 * 但是这样做并不会带来太多的损失: 因为当我们不在执行本任务时睡眠状态会自动释放中断(即其他任务会使用自己 EFLAGS 中的中断标志). 
 */
...
static void add_wait(struct task_struct ** wait_address, select_table * p) {
	...
	// 若设置过也立刻返回. 这个判断主要是针对管道文件描述符. 例如若一个管道在等待可以进行读操作, 那么其必定可以立刻进行写操作. 
	if (!wait_address)
		return;
	for (i = 0 ; i < p->nr ; i++)
		if (p->entry[i].wait_address == wait_address)
			return;
	// 然后我们把描述符对应等待队列的头指针保存在等待表 wait_table 中, 
	// 同时让等待表项的 old_task 字段指向等待队列头指针指向的任务(若无则为NULL), 
	// 在让等待队列头指针指向当前任务. 最后把等待表有效项计数值 nr 增 1(其在第 179 行初始化为 0). 
...
static void free_wait(select_table * p) {
	...
	// 如果等待表中各项(共 nr 个有效项)记录的等待队列头指针表明还有其他后来添加进的等待任务(例如其他进程调用 sleep_on() 函数而睡眠在该等待队列上), 
	// 则此时等待队列头指针指向的不是当前进程, 那么我们就需要先唤醒这些任务. 操作方法是将等待队列头所指任务先置为就绪状态(state = 0), 
	// 并把自己设置为不可中断等待状态, 即自己要等待这些后续进队列的任务被唤醒而执行时来唤醒本任务. 然后重新执行调度程序. 
	for (i = 0; i < p->nr ; i++) {
		...
		// 执行到这里, 说明等待表当前处理项中的等待队列头指针字段 wait_address 指向当前任务, 若它为空, 则表明调度有问题, 于是显示警告信息. 
		// 然后我们让等待队列头指针指向在我们前面进入队列的任务(第 76 行). 若此时该头指针确实指向一个任务而不是 NULL, 
		// 则说明队列中还有任务(*tpp 不为空), 于是将该任务设置成就绪状态, 唤醒之. 最后把等待表的有效表项计数字段 nr 清零. 
		if (!*tpp)
...
static struct tty_struct * get_tty(struct m_inode * inode) {
	...
	// 如果终端设备号小于 0, 表示进程没有控制终端或没有使用终端, 于是返回 NULL. 否则返回对应的 tty 结构指针. 
	if (major == 5)
		minor = current->tty;
	else
 ...
 */
/*
 * check_XX 函数用于检查一个文件. 我们知道该文件要么是管道文件, 要么是字符设备文件, 或者要么是一个 FIFO(FIFO) 还未实现. 
 */
...
static int check_out(select_table * wait, struct m_inode * inode) {
	...
	// 首先根据文件 i 节点调用 get_tty() 检测文件是不是一个 tty 终端(字符)设备文件, 
	// 如果是则检查该终端写缓冲队列 write_q 中是否有空间可写入, 若有则返回 1, 
	// 若没有空间则把当前任务添加到 write_q 等待队列 proc_list 上并返回 0. 
	// 如果是管道文件则判断目前管道中是否有空闲空间可写入字符, 若有则返回 1, 
	// 若没有(管道满)则把当前任务添加到管道 i 节点的等待队列上并返回 0. 
	if (tty = get_tty(inode))
		if (!FULL(tty->write_q))
			return 1;
		else
...
// do_select() 是内核执行 select() 系统调用的实际处理函数. 该函数首先检查描述符集中各个描述符的有效性, 
// 然后分别调用相关描述符集描述符检查函数 check_XX() 对每个描述符进行检查, 同时统计描述符集中当前已经准备好的描述符个数. 
...
int do_select(fd_set in, fd_set out, fd_set ex,
	...
	// 首先把 3 个描述符集进行或操作, 在 mask 中得到描述符集中有效符位屏蔽码. 然后循环判断当前进程各个描述符是否有效并且包含在描述符集内. 
	// 在循环中, 每判断完一个描述符就会把 mask 右移 1 位, 因此根据 mask 的最低有效位我们就可以判断相应描述符是否在用户给定的描述符集中. 
	// 有效的描述符应该是一个管道文件描述符, 或者是一个字符设备文件描述符, 或者是一个 FIFO 描述符, 其余类型的都作为无效描述符而返回 EBADF 错误. 
	mask = in | out | ex;
	for (i = 0 ; i < NR_OPEN ; i++, mask >>= 1) {
		...
		if (!current->filp[i])                                  // 若文件未打开, 则返回描述符值. 
			return -EBADF;
		if (!current->filp[i]->f_inode)                         // 若文件 i 节点指针为空, 则返回错误号. 
			return -EBADF;
	...
	// 下面循环检查 3 个描述符集中的各个描述符是否准备好(可以操作). 此时 mask 用作当前正在处理描述符的屏蔽码. 
	// 循环中的 3 个函数 check_in(), check_out() 和 check_ex() 分别用来判断描述符是否已经准备好. 
	// 若一个描述符已经准备好, 则在相关描述符集中设置对应位, 并且把已准备好描述符个数计数值 count 增 1. 
	// 第 186 行 for 循环语句中的 mask += mask 行将于 mask << 1. 
repeat:
	wait_table.nr = 0;
	*inp = *outp = *exp = 0;
	count = 0;
	...
	for (i = 0 ; i < NR_OPEN ; i++, mask += mask) {
		// 如果此时判断的描述符在读操作描述符集中, 并且该描述符已经准备好可以进行读操作, 
		// 则把该描述符在描述符集 in 中对应位置为 1, 同时把已准备好描述符个数计数值 count 增 1. 
		if (mask & in)
			if (check_in(&wait_table, current->filp[i]->f_inode)) {
				*inp |= mask;   								// 描述符集中设置对应位. 
				count++;        								// 已准备好描述符个数计数. 
			}
		// 如果此时判断的描述符在写操作描述符集中, 并且该描述符已经准备好可以进行写操作, 
		// 则把该描述符在描述符集 out 中对应位置为 1, 同时把已准备好描述符个数计数值 count 增 1. 
		if (mask & out)
			if (check_out(&wait_table, current->filp[i]->f_inode)) {
				*outp |= mask;
				count++;
			}
		// 如果此时判断的描述符在异常描述符集中, 并且该描述符已经有异常出现, 
		// 则把该描述符在描述符集 ex 中对应位置为 1, 同时把已准备好描述符个数计数值 count 增 1. 
		if (mask & ex)
	...
	// 在对进程所有描述符判断处理后, 若没有发现有已准备好的描述符(count == 0), 并且此时进程没有收到任何非阻塞信号, 
	// 并且此时有等待着描述符或者等待时间还没有超时, 那么我们就把当前进程状态设置成可中断睡眠状态, 然后执行调度函数去执行其他任务. 
	// 当内核又一次调度执行本任务时就调用 free_wait() 唤醒相关等待队列上本任务前后的任务, 
	// 然后跳转到 repeat 标号处再次重新检测是否有我们关心的(描述符集中的)描述符已准备好. 
 ...
 */
/*
 * 注意我们不能返回 -ERESTARTSYS, 因为我们会在 select 运行过程中改变输入参数值(*timeout). 
 // 很不幸, 但你也只能接受这个事实. 不过我们可以在库函数中做些处理...
 */
// select 系统调用函数. 该函数中的代码主要负责进行 select 功能操作前后的参数复制和转换工作. 
// select 主要的工作由 do_select() 函数来完成. 
// sys_select() 会首先根据参数传递来的缓冲区指针从用户数据空间把 select() 函数调用的参数分解复制到内核空间, 
...
int sys_select( unsigned long *buffer ) {
	...
	// 其中 mask 也是一个描述符集变量, 根据 3 个描述符集中最大描述符值 +1(即第 1 个参数 nd 的值), 
	// 它被设置成用户程序关心的所有描述符的屏蔽码. 例如, 若 nd = 4, 则 mask = 0b00001111(共 32 位). 
	...
	// 另外, 第 241 行上 tv_usec 字段是微秒值, 把它除以 1000000 后可得到对应秒数, 
	// 再乘以系统每秒嘀嗒数 HZ, 即把 tv_usec 转换成嘀嗒值. 
	timeout = 0xffffffff;
	if (tvp) {
	...
	// select() 函数的主要工作在 do_select() 中完成. 在调用该函数之后的代码用于把处理结果复制到用户数据区中, 返回给用户. 
	// 为了避免出现竞争条件, 在调用 do_select() 前需要禁止中断, 并在该函数返回后再开启中断. 
	// 如果在 do_select() 返回之后进程的等待延时字段 timeout 还大于当前系统计时嘀嗒值 jiffies, 说明在超时之前已经有描述准备好, 
	//于是这里我们先记下到超时还剩余的时间值, 随后我们会把这个值返回给用户. 如果进程的等待延时字段 timeout 已经小于或等于当前系统 jiffies, 
