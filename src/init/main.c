...
// TASK-0 使用 `int $0x80` 指令调用内核代码创建 TASK-1 时会发生特权级的变化, 所以会使用进程的内核态堆栈(ss0, esp0), 
// 而是使用 tss 中指定的内核态堆栈(tss0)(特权级 CPL[3 -> 0] 变化引起堆栈切换).
...
int main(void) {									/* This really IS void, no error here. */
	...
	time_init();									// 设置开机启动时间.
	// 将 int 0x80 中断陷阱门添加到 IDT 通过该操作完成.
	...
	// 下面通过在堆栈中构建 IRET 指令的返回参数, 利用中断返回指令切换到任务 0 中执行(在用户特权级下执行).
	 ...
	 */
	/*
	 * 注意!! 对于任何其他任务, 'pause()' 将意味着我们必须等待收到一个信号才会返回就绪态, 但任务 0 是唯一例外情况(参见 'schedule()'), 
