...
void do_hd_request(void)
{
	...
	// 并跳转到标号 repeat 处(定义在 INIT_REQUEST开始处).
	...
	// 然后根据求得的绝对扇区号 block 和硬盘号 dev, 我们就可以计算出对应硬盘中的磁道中扇区号(sec), 
	// 所在柱面号(cyl)和磁头号(head).
	...
	// 此时我们得到了欲读写的硬盘起始扇区 block 所对应的硬盘上柱面号(cyl), 
	// 在当前磁道上的扇区号(sec), 磁头号(head)以及欲读写的总扇区数(nsect). 
	// 接着我们可以根据这些信息向硬盘控制器发送 I/O 操作信息了. 
	// 但在发送之前我们还需要先看看是否有复位控制器状态和重新校正硬盘的标志. 
	// 通常在复位操作之后都需要重新校正硬盘磁头位置. 
