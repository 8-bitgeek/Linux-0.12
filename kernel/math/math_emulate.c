 ...
 */
/*
 * 仿真范围有限的程序 91.12.27 - 绝大多数是一些加载/存储指令. 除非你使用了 Bruce Evans 的补丁程序, 
 * 否则即使使用软件执行浮点运算, gcc 也需要这些指令. Bruce 的补丁程序非常好, 但每次换 gcc 版本你都得用这个补丁程序. 
 * 而且对于软件浮点实现和 80387, 所使用的库是不同的. 因此使用仿真是更为实际的方法, 尽管仿真方法更慢. 
 * 
 * 91.12.24 - 加载/存储协处理器指令可以用了, 即使是 BCD 码的也能使用. 我将开始考虑实现 add/sub/mul/div 指令. 
 * 唉, 我应该找一些好的资料, 不过现在我会先仿造一些操作. 
 * 
 * 91.12.30 - add/sub/mul/div/com 这些指令好像大多数可以使用了. 我真应该测试每种指令可能的组合操作. 
 */
 ...
 */
/*
 * 这个程序中到处到是些别扭的宏: 问题之一是 gcc 就是不想把结构建立成其应该成为的样子; 
 * gcc 企图对结构进行对齐处理. 真是讨厌, 不过我起码已经把所有蹩脚的代码都隐藏在这么一个文件中了, 其他程序文件不需要了解这些信息. 
 * 
 * 其他的程序也不需要知道 ST(x) 等 80387 内部结构 - 它们只需要得到 80 位临时实数的地址就可以随意操作. 
 * 我想尽可能在这里隐藏所有 387 专有信息. 
 */
...
#define bswapw(x) __asm__("xchgb %%al,%%ah":"=a" (x):"0" ((short)x))    // 交换 2 字节位置. 
#define ST(x) (*__st((x)))                              				// 取仿真的 ST(x) 累加器的值. 
...
// 执行浮点指令仿真. 
// 该函数首先检测仿真的 I387 结构状态字寄存器中是否有未屏蔽的异常标志置位. 若有则对状态字中忙标志B进行设置. 
...
// 该函数首先检测仿真的 I387 结构状态字寄存器中是否有未屏蔽的异常标志置位. 若有就设置状态字中忙标志 B(位 15), 
...
// 此时取出的代码正好与指令第 1, 第 2 字节顺序颠倒. 因此我们需要交换一下 code 中两个字节的顺序. 
// 然后再屏蔽掉第 1 个代码字节中的 ESC 位(二进制 11011). 接着把浮点指令指针 EIP 保存到 TSS 段 i387 结构中的 fip 字段中, 
...
// 处理第 2 个字节位 7--6 是 MOD, 位 2--0 是 R/M 的指令, 即 11011, XXX, MOD, XXX, R/M 形式的代码. 
// MOD 在各子程序中处理, 因此这里首先让代码与上 0xe7(0b11100111) 屏蔽掉 MOD. 
	switch ((code>>3) & 0xe7) {
		case 0x22:
...
// 下面处理第 2 类浮点指令. 首先根据指令代码的位 10--9 的 MF 值取指定类型的数, 然后根据 OPA 和 OPB 的组合值进行分别处理. 
