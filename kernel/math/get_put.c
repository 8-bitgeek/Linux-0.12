 ...
 */
/*
 * 本程序处理所有对用户内存的访问: 获取和存入指令 / 实数值 / BCD 数值等. 
 * 这是涉及临时实数以外其他格式仅有的部分. 所有其他运算全都使用临时实数格式. 
 */
...
// 取用户内存中的短实数(单精度实数). 
// 根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得短实数所在有效地址(math/ea.c), 
// 然后从用户数据区读取相应实数值. 最后把用户短实数转换成临时实数(math/convert.c). 
...
// 取用户内存中的长实数(双精度实数). 
// 首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得长实数所在有效地址(math/ec.c), 
// 然后从用户数据区读取相应实数值. 最后把用户实数值转换成临时实数(math/convert.c). 
// 参数: tmp - 转换成临时实数后的指针; info - info 结构指针; code - 指令代码. 
...
// 取用户内存中的临时实数. 
// 首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得临时实数所在有效地址(math/ea.c), 
// 然后从用户数据区读取相应临时实数值. 
// 参数: tmp - 转换成临时实数后的指针; info - info 结构指针; code - 指令代码. 
...
// 取得短整数所在有效地址(math/ea.c), 然后从用户数据区读取相应整数值, 并保存为临时整数格式. 
// 最后把临时整数值转换成临时实数(math/convert.c). 
// 参数: tmp - 转换成临时实数后的指针; info - info 结构指针; code - 指令代码. 
...
// 首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得长整数所在有效地址(math/ea.c), 
...
// 首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得 64 位长整数所有有效地址(math/ea.c), 
...
// 该函数首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 取得 BCD 码所在有效地址(math/ea.c), 
// 然后从用户数据区读取 10 字节相应 BCD 码值(其中 1 字节用于符号), 同时转换成临时整数形式. 最后把临时整数值转换成临时实数. 
// 参数: tmp - 转换成临时实数后的指针; info - info 结构指针; code - 指令代码. 
...
// 先取得 BCD 码数值的符号位, 并设置临时整数的符号位. 然后把 9 字节的 BCD 码值转换成临时整数格式, 
// 最后把临时整数值转换成临时实数. 
...
// 该函数首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 
// 取得保存结果的有效地址 addr, 然后把临时实数格式的结果转换成短实数格式并存储到有效地址 addr 处. 
...
// 该函数首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 
// 取得保存结果的有效地址 addr, 然后把临时实数格式的结果转换成长实数格式, 并存储到有效地址 addr 处. 
// 参数: tmp - 临时实数格式结果值; info - info 结构指针; code - 指令代码. 
...
// 该函数首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 
// 取得保存结果的有效地址 addr, 然后把临时实数格式的结果转换成临时整数格式. 
// 如果是负数则设置整数符号位. 最后把整数保存到用户内存中. 
// 参数: tmp - 临时实数格式结果值; info - info 结构指针; code - 指令代码. 
...
// 该函数首先根据浮点指令代码中寻址模式字节中的内容和 info 结构中当前寄存器中的内容, 
// 取得保存结果的有效地址 addr, 并验证保存 10 字节BCD码的用户空间. 
// 然后把临时实数格式的结果转换成 BCD 码格式的数据并保存到用户内存中. 
// 如果是负数则设置最高存储字节的最高有效位. 
// 参数: tmp - 临时实数格式结果值; info - info 结构指针; code - 指令代码. 
void put_BCD(const temp_real * tmp, struct info * info, unsigned short code) {
	...
	for (k = 0; k < 9; k++) {               	// 临时整数转换成 BCD 码并保存. 
		DIV10(i.a, i.b, rem);
		c = rem;
		DIV10(i.a, i.b, rem);
		c += rem << 4;
