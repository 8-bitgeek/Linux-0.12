...
void release(struct task_struct * p)
{
	...
	// 扫描任务结构指针数组, 寻找指定的任务 p. 如果找到, 则置空任务指针数组中对应项, 并且更新任务结构之间的关联指针, 
	// 释放任务 p 数据结构占用的内在页面. 最后在执行调度程序返回后退出. 如果没有找到指定的任务 p, 则说明内核代码出错了, 
	// 则显示出错信息并死机. 
...
void audit_ptree()
{
	...
	for (i = 1 ; i < NR_TASKS ; i++) {
		if (!task[i])
			continue;
		// 如果任务的父进程指针 p_pptr 没的指向任何进程(即在任务数组中存在), 则显示警告信息 "警告, pid 号 N 的父进程链接有问题". 
