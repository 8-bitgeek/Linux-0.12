...
// ESgetparts- 该状态表示此时要接收控制序列的参数值. 参数用十进制数表示, 把接收到的数字字符转换成数值并保存到 par[] 数组中.
// 			   如果收到一个分号';', 则还是维持在本状态, 并把接收到的参数值保存在数据 par[] 下一项中. 
// 			   若不是数字字符或分号, 说明已取得所有参数, 那么就转移到状态 ESgotparts 去处理.
// ESgotparts- 表示已经接收到一个完整的控制序列. 此时可以根据本状态接收到的结尾字符对相应控制序列进行处理. 
// 			   不过在处理之前, 如果在 ESsquare 状态收到过 '?', 说明这个序列是终端设备私有序列. 
// 			   本内核不对支持对这种序列的处理, 于是直接恢复到 ESnormal 状态. 否则就去执行相应控制序列.
//             待序列处理完后就把状态恢复到 ESnormal.
// ESfunckey - 表示接收到了键盘上功能键发出的一个序列, 不用显示. 于是恢复到正常状态 ESnormal.
// ESsetterm - 表示处于设备控制字符串序列状态(DCS). 此时收到字符 'S', 则恢复初始的显示字符属性. 
// 			   若收到的字符是 'L' 或 'l', 则开启或关折行显示方式.
...
void con_write(struct tty_struct * tty)
{
	...
	// 该函数首先根据当前控制台使用的 tty 在 tty 表的项位置取得对应控制台号 currcons, 
	// 然后计算出 (CHARS()) 目前 tty 写队列中含有的字符数 nr, 并循环取出其中的每个字符进行处理. 
	// 不过如果当前控制台由于接收键盘或发出的暂停命令(如按键 Ctrl-S)而处于停止状态, 
	// 那么本函数就停止处理写队列中的字符, 退出函数. 
	// 另外, 如果取出的是控制字符 CAN(24) 或 SUB(6), 那么若是在转义或控制序列期间收到的, 
	...
	while (nr--) {
		...
		switch(state) {
			...
			// 并放到当前光标所处的显示内存位置处, 即直接显示该字符. 然后把光标位置右移一个字符位置. 
			// 具体地, 如果字符不是控制字符也不是扩展字符, 即(31<c<127), 那么, 若当前光标处在行末端或末端以外, 
			// 则将光标移到下行头列. 并调整光标位置对应的内存指针 pos. 
			// 然后将字符 c 写到显示内存中 pos 处, 并将光标右移 1 列, 同时也将 pos 对应地移动 2 个字节.
			case ESnormal:
				...
				// 如果字符 c 是水平制表符 HT(9), 则将光标移到 8 的位数列上. 
			...
			case ESesc:
				...
				break;
			// ESsquare: 表示已经接收到一个控制序列引导码 ('ESC ['), 表示接收到的是一个控制序列. 
			// 			于是本状态执行参数数组 par[] 清零初始化工作. 如果此时接收到的又是一个 '[' 字符, 则示收到了 'ESC [[' 序列. 
			// 			该序列是键盘功能键发出的序列, 于是跳转到 ESfunckey 去处理.
			// 			否则我们需要准备接收控制序列的参数, 于是置状态 ESgetparts 并直接进入该状态去接收并保存序列的参数字符.
			// 如果在状态 ESesc(是转义字符 ESC) 时收到的字符 '[', 则表明是 CSI 控制序列, 于是转到状态 Essequare 来处理. 
			// 首先对 ESC 转义序列保存参数数组 par[] 清零, 索引变量 npar 指向首项, 并且设置开始处于参数状态 ESgetpars. 
			// 如果接收到的字符不是 '?', 则直接转到状态 ESgetpars 去处理, 
			// 若接收到的字符是 '?', 说明这个序列是终端设备私有序列, 后面会有一个功能字符. 
			...
			// ESgotpars 状态表示我们已经接收到一个完整的控制序列. 此时可以根据本状态接收到的结尾字符对相应控制序列进行处理. 
			//			 不过在处理之前, 如果在 ESsquare 状态收到过 '?', 说明这个序列是终端设备私有序列. 
			//           本内核不支持对这种序列的处理, 于是直接恢复到 ESnormal 状态. 
			//  		 否则就去执行相应控制序列. 待序列处理完后就把状态恢复到 ESnormal.
			case ESgotpars:
				state = ESnormal;
				if (ques)
				...
				switch(c) {
					...
					case 'u':									// CSI u - 恢复保存的光标位置.
						restore_cur(currcons);
						break;
					// 如果字符 c 是 'l' 或 'b', 则分别表示设置屏幕黑屏间隔时间和设置粗体字符显示. 
					// 此时参数数组中 par[1] 和 par[2] 是特征值, 它们分别必须 par[1] = par[0] + 13; 
					// par[2] = par[0] + 17. 在这个条件下, 如果 c 是字符 'l', 
			...
			case ESfunckey:									// 键盘功能键码.
				state = ESnormal;
				break;
			// ESsetterm: 表示处于设备控制字符串序列状态(DCS). 
			// 此时收到字符 'S', 则恢复初始的显示字符属性. 若收到的字符是 'L' 或 'l', 则开启或关折行显示方式.
