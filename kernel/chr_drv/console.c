 ...
 *
 * 虚拟控制台, 屏幕黑屏处理, 屏幕拷贝, 彩色处理, 图形字符显示以及 VT100 终端增强操作由 Peter MacDonald 编制.
 */
 ...
 */
/*
 * 注意!!! 我们有时短暂地禁止和允许中断(当输出一个字(word)到视频 IO), 但即使对于键盘中断这也是可以工作的. 
 * 因为我们使用了陷阱门所以我们知道在处理一个键盘过程期间中断是被禁止的. 希望一切均正常.
 */
...
// 主机通过发送不带参数或参数是 0 的设置属性(DA)控制序列('ESC [c' 或 'ESC [0c'] 要求终端应答一个设备属性控制序列(ESC Z 的作用与此相同)
// 终端则发送以下序列来响应主机. 该序列(即 'ESC [?1;2c')表示终端是具有高级视频功能的 VT100 兼容终端.
#define RESPONSE "\033[?1;2c"

// 定义使用的字符集. 其中上半部分时普通 7 位 ASCII 码, 即 US 字符集. 下半部分对应 VT100 终端设备中的线条字符, 即显示图表线条的字符集.
...
// 更新当前光标位置变量 x, y, 并修正光标在显示内存中的对应位置 pos. 该函数会首先检查参数的有效性. 
// 如果给定的光标列号超出显示器最大列数, 或者光标行号不低于显示的最大行数, 则退出. 
// 否则就更新当前光标变量和新光标位置对应在显示内存中位置 pos.
...
static void scrup(int currcons)
{
	...
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
	{
		...
		if (!top && bottom == video_num_lines) {
			...
			set_origin(currcons);
		// 否则表示不是整屏移动. 即表示从指定行 top 开始到 bottom 区域中的所有行向上移动 1 行, 指定行 top 被删除. 
		// 此时直接将屏幕从指定行 top 到屏幕末端所有行对应的显示内存数据向上移动 1 行, 并在最下面新出现的行上填入擦除字符.
...
// 向下卷动一行
// 将屏幕滚动窗口向上移动一行, 相应屏幕滚动区域内容向下移动 1 行. 并在移动开始行的上方出现一新行. 
// 处理方法与 scrup() 相似, 只是为了在移动显示内存数据时不会出现数据覆盖的问题, 复制操作是以逆向进行的, 
...
// 删除屏幕上与光标位置相关的部分
// ANSI控制序列: 'ESC [ Ps J'(Ps = 0 - 删除光标处到屏幕底端; 1 - 删除屏幕开始到光标处; 2 - 整屏删除). 
...
static void csi_J(int currcons, int vpar)
{
	...
	// 然后使用擦除字符填写被删除字符的地方.
	// %0 - ecx(删除的字符数 count); %1 - edi(删除操作开始的地址); %2 - eax(填入的擦除字符).
	__asm__("cld\n\t"
		"rep\n\t"
		"stosw\n\t"
		::"c" (count),
...
void csi_m(int currcons)
{
	...
	// 如果是彩色显示, 那么若原来 vc_bold_attr 不等于 -1 时就复位其背景色; 否则的话就把背景色取反. 
	// 若取反后前景色与背景色相同, 就把前景色增1而取另一种颜色.
	for (i = 0; i <= npar; i++)
		switch (par[i]) {
			case 0:
			...
			// 如果 Ps = 24, 则对于单色显示是取消随后字符的下划线显示, 对于彩色显示取是取消绿色.
...
static void insert_line(int currcons)
{
	...
	// 最后恢复屏幕窗口卷动开始行 top 和最后行 bottom 的原来值.
	oldtop = top;
	oldbottom = bottom;
	top = y;										// 设置屏幕卷动开始行和结束行.
...
// 删除光标处的 nr 行.
// ANSI 转义序列: 'ESC [ Pn M'. 该控制序列在滚动区域内, 从光标所在行开始删除 1 行或多行. 
...
// 这个枚举定义用于下面 con_write() 函数中处理转义序列或控制序列的解析. ESnormal 是初始进入状态, 也是转义或控制序列处理完毕时的状态.
// ESnormal  - 表示处于初始正常状态. 此时若接收到的是普通显示字符, 则把字符直接显示在屏幕上; 若接收到的是控制字符(例如回车字符), 
// 			   则对光标位置进行设置. 当刚处理完一个转义或控制序列, 程序也会返回到本状态.
...
// 			   使用单独的状态 ESsetgraph 来处理; 对于设备控制字符串序列 'ESC P', 使用单独的状态 ESsetterm 来处理.
// ESsquare  - 表示已经接收到一个控制序列引导码('ESC ['), 表示接收到的是一个控制序列. 于是本状态执行参数数组 par[] 清零初始化工作. 
// 			   如果此时接收到的又是一个 '[' 字符, 则示收到了 'ESC [[' 序列. 该序列是键盘功能键发出的序列, 于是跳转到 ESfunckey 去处理. 
// 			   否则我们需要准备接收控制序列的参数, 于是置状态 ESgetparts 并直接进入该状态去接收并保存序列的参数字符.
// ESgetparts- 该状态表示此时要接收控制序列的参数值. 参数用十进制数表示, 把接收到的数字字符转换成数值并保存到 par[] 数组中.如果收到一个分号';',
//             则还是维持在本状态, 并把接收到的参数值保存在数据 par[] 下一项中. 若不是数字字符或分号, 说明已取得所有参数, 那么就转移到状态 ESgotparts 去处理.
// ESgotparts- 表示已经接收到一个完整的控制序列. 此时可以根据本状态接收到的结尾字符对相应控制序列进行处理. 不过在处理之前, 如果在 ESsquare 状态收到过'?', 
//  		   说明这个序列是终端设备私有序列. 本内核不对支持对这种序列的处理, 于是直接恢复到 ESnormal 状态. 否则就去执行相应控制序列.
//             待序列处理完后就把状态恢复到 ESnormal.
...
// ESsetgraph- 表示收到设置字符转移序列 'ESC (' 或 'ESC )'. 它们分别用于指定 G0 和 G1 所用的字符集. 此时若收到字符 '0', 则选择图形字符集作为 G0 和 G1,
...
void con_write(struct tty_struct * tty)
{
	...
	// 该函数首先根据当前控制台使用的 tty 在 tty 表的项位置取得对应控制台号 currcons, 然后计算出 (CHARS()) 目前 tty 写队列中含有的字符数 nr, 
	// 并循环取出其中的每个字符进行处理. 不过如果当前控制台由于接收键盘或发出的暂停命令(如按键 Ctrl-S)而处于停止状态, 
	// 那么本函数就停止处理写队列中的字符, 退出函数. 另外, 如果取出的是控制字符 CAN(24) 或 SUB(6), 那么若是在转义或控制序列期间收到的, 
	// 则序列不会执行而立刻终止, 同时显示随后的字符. 注意, con_write() 函数只处理取队列字符数时写队列中当前含有的字符. 
	...
	while (nr--) {
		...
		switch(state) {
			// ESnormal: 表示处于初始正常状态. 此时若接收到的是普通显示字符, 则把字符直接显示在屏幕上; 若接收到的是控制字符(例如回车字符), 
			// 			则对光标位置进行设置. 当刚处理完一个转义或控制序列, 程序也会返回到本状态.
			// 如果从写队列中取出的字符是普通显示字符代码, 就直接从当前映射字符集中取出对应的显示字符, 并放到当前光标所处的显示内存位置处, 即直接显示该字符. 
			// 然后把光标位置右移一个字符位置. 具体地, 如果字符不是控制字符也不是扩展字符, 即(31<c<127), 那么, 若当前光标处在行末端或末端以外, 
			// 则将光标移到下行头列. 并调整光标位置对应的内存指针 pos. 然后将字符 c 写到显示内存中 pos 处, 并将光标右移 1 列, 同时也将 pos 对应地移动 2 个字节.
			case ESnormal:
				...
				// 如果字符 c 是水平制表符 HT(9), 则将光标移到 8 的位数列上. 若此时光标列数超出屏幕最大列数, 则将光标移到下一行上.
				} else if (c == 9) {								// HT - 水平制表
					c = 8 - (x & 7);
					x += c;
					pos += c << 1;
			...
			// 		 于是跳转到 ESsquare 去处理. 否则就把接收到的字符作为转义序列来处理. 对于选择字符集转义序列 'ESC (' 和 'ESC )', 
			// 		 使用单独的状态 ESsetgraph 来处理; 对于设备控制字符串序列 'ESC P', 使用单独的状态 ESsetterm 来处理.
			// 如果在 ESnormal 状态收到转义字符 ESC(0x1b = 033 = 27), 则转到本状态处理. 该状态对 C1 中控制字符或转义字符进行处理. 
			...
			// 			如果此时接收到的又是一个 '[' 字符, 则示收到了 'ESC [[' 序列. 该序列是键盘功能键发出的序列, 于是跳转到 ESfunckey 去处理.
			// 			否则我们需要准备接收控制序列的参数, 于是置状态 ESgetparts 并直接进入该状态去接收并保存序列的参数字符.
			// 如果在状态 ESesc(是转义字符 ESC) 时收到的字符 '[', 则表明是 CSI 控制序列, 于是转到状态 Essequare 来处理. 
			// 首先对 ESC 转义序列保存参数数组 par[] 清零, 索引变量 npar 指向首项, 并且设置开始处于参数状态 ESgetpars. 如果接收到的字符不是 '?', 
			// 则直接转到状态 ESgetpars 去处理, 若接收到的字符是 '?', 说明这个序列是终端设备私有序列, 后面会有一个功能字符. 
			// 于是去读下一字符, 再到状态 ESgetpars 去处理代码处. 如果此时接收到字符还是 '[', 那么表明收到了键盘功能键发出的序列, 于是设置下一状态为 ESfunckey. 
			...
			// 			  如果收到一个分号 ';', 则还是维持在本状态, 并把接收到的参数值保存在数据 par[] 下一项中. 若不是数字字符或分号, 
			//			  说明已取得所有参数, 那么就转移到状态 ESgotparts 去处理.
			// 该状态表示此时要接收控制序列的参数值. 参数用十进制数表示, 把接收到的数字字符转换成数值并保存到 par[] 数组中. 
			// 如果收到一个分号 ';', 则还是维持在本状态, 并把接收到的参数值保存在数据 par[] 下一项中. 
			// 若不是数字字符或分号, 说明已取得所有参数, 那么就转移到状态 ESgotpars 去处理.
			case ESgetpars:
				if (c == ';' && npar < NPAR - 1) {
					npar++;
					break;
			...
			// 			 如果在 ESsquare 状态收到过 '?', 说明这个序列是终端设备私有序列. 本内核不对支持对这种序列的处理, 于是直接恢复到 ESnormal 状态.
			// 			 否则就去执行相应控制序列. 待序列处理完后就把状态恢复到 ESnormal.
			// ESgotpars 状态表示我们已经接收到一个完整的控制序列. 此时可以根据本状态接收到的结尾字符对相应控制序列进行处理. 不过在处理之前, 
			// 如果在 ESsquare 状态收到过 '?', 说明这个序列是终端设备私有序列. 本内核不支持对这种序列的处理, 于是直接恢复到 ESnormal 状态. 
			// 否则就去执行相应控制序列. 待序列处理完后就把状态恢复到 ESnormal.
			case ESgotpars:
				state = ESnormal;
				if (ques)
				...
				switch(c) {
					...
					case 'A':									// CSI Pn A - 光标上移.
						...
						break;
					// 如果 c 是 'B' 或 'e', 则第 1 个代表光标右移的格数. 若参数为 0 则下移一行.
					...
					// 序列: 'ESC [ Ps J'(Ps = 0 删除光标到屏幕底端; Ps = 1 删除屏幕开始到光标处; Ps = 2整屏删除).
					case 'J':									// CSI Pn J - 屏幕擦除字符.
						csi_J(currcons, par[0]);
						break;
					// 如果字符 c 是 'K', 则第 1 个参数代表以光标所在位置对行中字符进行删除处理的方式:
					// 序列: 'ESC [ Ps K'(Ps = 0 删除到行尾; Ps = 1 从开始删除; Ps = 2 整行都删除).
					...
					case 'm':									// CSI Ps m - 设置显示字符属性.
						csi_m(currcons);
						break;
					// 如果字符 c 是 'r', 则表示两个参数设置滚屏的起始行号和终止行号.
					...
					case 'u':									// CSI u - 恢复保存的光标位置.
						restore_cur(currcons);
						break;
					// 如果字符 c 是 'l' 或 'b', 则分别表示设置屏幕黑屏间隔时间和设置粗体字符显示. 此时参数数组中 par[1] 和 par[2] 是特征值, 
					// 它们分别必须 par[1] = par[0] + 13; par[2] = par[0] + 17. 在这个条件下, 如果 c 是字符 'l', 
				...
				break;
			// ESfunckey: 表示接收到了键盘上功能键发出的一个序列, 不用显示. 于是恢复到正常状态 ESnormal.
			// 状态 ESfunckey 表示接收到了键盘上功能键发出的一个序列, 不用显示. 于是恢复到正常状态 ESnormal.
			case ESfunckey:									// 键盘功能键码.
				state = ESnormal;
				break;
			// ESsetterm: 表示处于设备控制字符串序列状态(DCS). 此时收到字符 'S', 则恢复初始的显示字符属性. 若收到的字符是 'L' 或 'l', 则开启或关折行显示方式.
			// 状态 ESsetterm 表示处于设备控制字符串序列状态(DCS). 此时若收到字符 'S', 则恢复初始的显示字符属性. 
			...
			// 			   此时若收到字符 '0', 则选择图形字符集作为 G0 和 G1, 若收到的字符是 'B', 这选择普通 ASCII 字符集作为 G0 和 G1 的字符集.
			// 状态 ESsetgraph 表示收到设置字符集转移序列 'ESC (' 或 'ESC )'. 它们分别用于指定 G0 和 G1 所用的字符集. 
...
void con_init(void)
{
	...
	// 设置此时第 0 个(currcons)显示终端的擦除字符属性及字符
	...
	if (ORIG_VIDEO_MODE == 7)					/* Is this a monochrome display? */
	{
		...
		// 接着根据 BIOS 中断 int 0x10 功能 0x12 获得的显示模式信息, 判断显示卡是单色显示卡还是彩色显示卡. 
		// 若用上述中断功能所得到的 BX 寄存器返回值不等于 0x10, 则说明是 EGA 卡. 因此初始显示类型为 EGA 单色. 
		// 虽然 EGA 卡上有较多显示内存, 但在单色方式下最多只能利用地址范围在 0xb0000~xb8000 之间的显示内存.
		// 然后置显示器描述字符串为 'EGAm'.
...
static void sysbeep(void)
{
	...
	/* send 0x637 for 750 HZ */						/* 设置频率为 720Hz, 因此送定时值 0x637 */
	outb_p(0x37, 0x42);								// 通过 2 数据端口分别送计数高低字节
	outb(0x06, 0x42);
	/* 1/8 second */								/* 蜂鸣时间为 1/8s */
	beepcount = HZ / 8;
}
