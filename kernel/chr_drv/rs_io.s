...
# 当写缓冲队列中还剩余最多 256 个字符时, 中断处理程序就可以唤醒这些等待进程继续往写队列中放字符. 
...
# 这里首先把 tty 表中串行终端 1(串口 1) 读写缓冲队列指针的地址入栈(tty_io.c), 然后跳转到 rs_int 继续处理. 
# 这样做可以让串口 1 和串口 2 的处理代码公用. 字符缓冲队列结构 tty_queue 格式请参见 include/linux/tty.h. 
.align 4
rs1_interrupt:
	pushl $table_list + 8         								# tty 表中串口 1 读写缓冲队列指针地址入栈. 
	jmp rs_int
.align 4
rs2_interrupt:
	pushl $table_list + 16        								# tty 表中串口 2 读写缓冲队列指针地址入栈. 

# 这段代码首先让段寄存器 ds, es 指向内核数据段, 然后从对应读写缓冲队列 data 字段取出串行端口基地址. 
# 该地址加2即是中断标识寄存器 IIR 的端口地址. 若位 0 = 0, 表示有需要处理的中断. 于是根据位 2, 位 1 使用指针跳转表调用相应中断源类型处理子程序. 
# 在每个子程序中会在处理完后复位 UART 的相应中断源. 在子程序返回后这段代码会循环判断是否还有其他中断源(位 0 = 0?). 
# 如果本次中断还有其他中断源, 则 IIR 的位 0 仍然是 0. 于是中断处理程序会再调用相应中断源子程序继续处理. 
# 直到引起本次中断的所有中断源都被处理并复位, 此时 UART 会自动地设置 IIR 的位 0 = 1, 表示已无待处理的中断, 于是中断处理程序即可退出. 
rs_int:
	pushl %edx
	pushl %ecx
	pushl %ebx
...
rep_int:
	...
	movl 24(%esp), %ecx      									# 调用子程序之前所缓冲队列指针地址放入 ecx. 
	pushl %edx              									# 临时保存中断标识寄存器端口地址. 
	subl $2, %edx            									# edx 中恢复串口基地址值 0x3f8(0x2f8). 
	call *jmp_table(, %eax, 2)									/* NOTE! not *4, bit0 is 0 already */
	# 上面语句是指, 当有待处理中断时, al 中位 0 = 0, 位 2, 位 1 是中断类型, 因此相当于已经将中断类型乘了 2, 这里再乘 2, 
	# 获得跳转表对应各个中断类型地址, 并跳转到那里去作相应处理. 中断来源有 4 种: modem 状态发生变化; 要写(发送)字符; 
	# 要读(接收)字符; 线路状态发生变化. 允许发送字符中断通过设置发送保持寄存器标志实现. 
	# 在 serial.c 程序中, 当写缓冲队列中有数据时, rs_write() 函数就会修改中断允许寄存器内容, 
	# 添加上发送保持寄存器中断允许标志, 从而在系统需要发送字符时引起串行中断发生. 
	popl %edx               									# 恢复中断标识寄存器端口地址 0x3fa(0x2fa). 
...
# 由于 mode 状态发生变化而引发此次中断. 通过读 modem 状态寄存器 MSR 对其进行复位操作. 
.align 4
modem_status:
...
# 若 head 指针已经到达缓冲区末端, 则让其折返到缓冲区开始处. 最后调用 C 函数 do_tty_interrupt()(即 copy_to_cooked()), 
# 把读入的字符经过处理放入规范模式缓冲队列(辅助缓冲队列 secondary)中. 
.align 4
read_char:
...
1:	addl $63, %edx                   							# 串口号转换成 tty 号(63 或 64)并作为参数入栈. 
	pushl %edx
	call do_tty_interrupt           							# 调用 tty 中断处理 C 函数(tty_io.c). 
	addl $4, %esp                    							# 丢弃入栈参数, 并返回. 
	ret

# 由设置了发送保持寄存器允许中断标志而引起此次中断. 说明对应串行终端的写字符缓冲队列中有字符需要发送. 
# 于是计算出写队列中当前所含字符数, 若字符数已小于 256 个, 则唤醒等待写操作进程. 
# 然后从写缓冲队列尾部取出一个字符发送, 并调整和保存尾指针. 如果写缓冲队列已空, 则跳转到 write_buffer_empty 处处理写缓冲队列空的情况. 
.align 4
write_char:
...
# 处理写缓冲队列 write_q 已空的情况. 若有等待写该串行终端的进程则唤醒之, 然后屏蔽发送保持寄存器中断, 
# 不让发送保持寄存器空时产生中断. 
