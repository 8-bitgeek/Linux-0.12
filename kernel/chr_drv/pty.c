...
static inline void pty_copy(struct tty_struct * from, struct tty_struct * to)
{
	...
	while (!from->stopped && !EMPTY(from->write_q)) {
		// 判断目的终端读队列是否已满, 如果已满则先调用 copy_to_cooked 函数处理目的终端读队列, 然后重新循环处理. 
		if (FULL(to->read_q)) {
		...
		GETCH(from->write_q, c);         					// 从源终端写队列中取一个字符, 放入 c, 
		PUTCH(c, to->read_q);            					// 然后把 c 中的字符放入目的终端读队列中. 
 ...
 */
/*
 * 这个函数被调用时, tty_write 函数已经把一些字符放到写队列 write_queue 中. 
 * 它将复制这些输入到它的从伪终端的输出队列中. 
 */
// 主伪终端写函数. 
void mpty_write(struct tty_struct * tty)
{
	...
	// 终端号除以 64 取整如果为 2 则是 "主伪终端", 否则显示出错信息. 
	if ((nr >> 6) != 2)
		printk("bad mpty\n\r");
	else
...
void spty_write(struct tty_struct * tty)
{
	...
	// 终端号除以 64 取整如果为 3 则是 "从伪终端", 否则显示出错信息. 
	if ((nr >> 6) != 3)
		printk("bad spty\n\r");
	else
		pty_copy(tty, tty - 64);   							// 调用伪终端写函数. 
