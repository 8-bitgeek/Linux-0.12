 ...
 */
/*
 * 'tty_io.c' 给 tty 终端一种非相关的感觉, 不管它们是控制台还是品串行终端.
 * 该程序同样实现了回显, 规范(熟)模式等.
 */
...
// 复制成规范模式字符序列
// 根据终端 termios 结构中设置的各种标志, 将指定 tty 同读队列缓冲区中的字符复制转换成规范模式(熟模式)字符并存放在辅助队列(规范模式队列)中.
...
void copy_to_cooked(struct tty_struct * tty)
{
	...
	// 否则我们根据终端 termios 结构中的输入和本地标志, 对从 tty 读队列缓冲区中取出的每个字符进行适当的处理, 
	// 然后放入辅助队列 secondary 中. 在下面循环体中, 如果此时读队列缓冲区已经取空或都辅助队列缓冲区已经放满字符, 就退出循环体. 
	// 否则程序就从读队列缓冲区尾指针处取一字符, 并把尾指针前移一个字符位置. 然后根据该字符代码值进行处理.
	// 另外, 如果定义了 _POSIX_VDISABLE(\0), 那么在对字符处理过程中, 若字符代码值等于 _POSIX_VDISABLE 的值时, 表示禁止使用相应特殊控制字符的功能.
	while (1) {
		...
		// 如果设置了 IXON 标志, 则使终端停止/开始输出控制字符起作用. 如果没有设置此标志, 那么停止和开始字符将被作为一般字符供进程读取. 
		// 在这段代码中, 如果读取的字符是停止字符 STOP(^S), 则置 tty 停止标志, 让 tty 暂停输出. 同时丢弃该特殊控制字符(不放入辅助队列中),
		// 并继续处理其他字符. 如果字符是开始字符 START(^Q), 则复位 tty 停止标志, 恢复 tty 输出. 同时丢弃该控制字符, 并继续处理其他字符. 
		...
		// 若输入模式标志集中 ISIG 标志置位, 表示终端键盘可以产生信号, 则在收到控制字符 INTR, QUIT, SUSP 或 DSUSP 时, 
		// 需要为进程产生相应的信号. 如果该字符是键盘中断符(^C), 则向当前进程之进程组中所有进程发送键盘中断信号 SIGINT, 并继续处理下一字符. 
		// 如果该字符是退出符(^\), 则向当前进程之进程组中所有进程发送键盘退出信号 SIGQUIT, 并继续处理下一字符. 如果字符是暂停符(^Z), 
		// 则向当前进程发送暂停信息 SIGTSTP. 同样, 若定义了 _POSIX_VDISABLE(\0), 那么在对字符处理过程中, 
		// 若字符代码值等于 _POSIX_VDISABLE 的值时, 表示禁止使用相应特殊控制字符的功能.
		if (L_ISIG(tty)) {
		...
		// 如果该字符是换行符 NL(10), 或者是文件结束符 EOF(4,^D), 表示一行字符已处理完, 则把辅助缓冲队列中当前含有字符行数值 secondar. 
		// data 增 1. 如果在函数 tty_read() 中取走一行字符, 该值即会减 1.
		if (c == 10 || (EOF_CHAR(tty) != _POSIX_VDISABLE && c == EOF_CHAR(tty)))
			tty->secondary->data++;
		// 如果本地模式标志中回显标志 ECHO 在置位状态, 那么, 如果字符是换行符 NL(10), 则将换行符 NL(10) 和回车符(13) 放入 tty 写队列缓冲区中; 
		// 如果字符是控制字符(值 < 32)并且回显控制字符标志 ECHOCTL 置位, 则将字符 '^' 和字符 c + 64 放入 tty 写队列中(也即会显示 ^C, ^H 等); 
...
int tty_read(unsigned channel, char * buf, int nr)
{
	...
	// 首先判断参数有效性并取终端的 tty 结构指针. 如果 tty 终端的三个缓冲队列指针都是 NULL, 则返回 EIO 出错信息. 
	...
	if (!(tty->write_q || tty->read_q || tty->secondary))
		return -EIO;
	// 如果当前进程使用的是这里正在处理的 tty 终端, 但该终端的进程组号却与当前进程组号不同, 表示当前进程是后台进程组中的一个进程, 即进程不在前台. 
	// 于是我们要停止当前进程组的所有进程. 因此这里就需要向当前进程组发送 SIGTTIN 信号, 并返回等待成为前台进程组后再执行读操作. 
	...
	// 在非规范模式下, 这两个是超时定时值. VMIN 表示为了满足读操作而需要读取的最少字符个数. VTIME 是一个 1/10 秒计数计时值. 
	...
	while (nr > 0) {
		...
		// 如果该字符是文件结束符(^D)并且规范模式标志成置位状态, 则中断本循环, 否则说明现在还没有遇到文件结束符或者正处于原始(非规范)模式. 
		// 在这种模式中用户以字符流作为读取对象, 也不识别其中的控制字符(如文件结束符). 于是将字符直接放入用户数据缓冲区 buf 中, 并把欲读字符数减 1.
		// 此时如果欲读字符数已为 0 则中断循环. 另外, 如果终端处于规范模式并且读取的字符是换行符 NL(10), 则也退出循环. 
		...
		// 执行到此, 那么如果 tty 终端处于规范模式下, 说明我们可能读到了换行符或者遇到了文件结束符. 如果是处于非规范模式下, 
		// 那么说明我们已经读取了 nr 个字符, 或者辅助队列已经被取空了. 于是我们首先唤醒等待队列的进程, 然后看看是否设置过超时定时值 time. 
		// 如果超时定时值 time 不为 0, 我们就要求等待一定的时间让其他进程可以把字符写入读队列中. 
		// 于是设置进程读超时定时值为系统当前时间 jiffies + 读超时值 time. 当然, 如果终端处于规范模式, 或者已经读取 nr 个字符, 
		// 我们就可以直接退出这个大循环了. 
...
int tty_write(unsigned channel, char * buf, int nr)
{
	...
	if (!(tty->write_q || tty->read_q || tty->secondary))
		return -EIO;
	// 如果若终端本地模式标志集中设置了 TOSTOP, 表示后台进程输出时需要发送信号 SIGTTOU. 如果当前进程使用的是这里正在处理的 tty 终端, 
	// 但该终端的进程组号却与当前进程组号不同, 即表示当前进程是后台进程组中的一个进程, 即进程不在前台. 
	// 于是我们要停止当前进程组的所有进程. 因此这里就需要向当前进程组发送 SIGTTOU 信号, 并返回等待成为前台进程组后再执行写操作.
	if (L_TOSTOP(tty) &&
	    (current->tty == channel) && (tty->pgrp != current->pgrp))
		return(tty_signal(SIGTTOU, tty));
	// 现在我们开始从用户缓冲区 buf 中循环取出字符并放到写队列缓冲区中. 当欲写字节数大于 0, 则执行以下循环操作. 
	// 在循环过程中, 如果此时 tty 写队列已满, 则当前进程进入可中断睡眠状态. 如果当前进程有信号要处理, 则退出循环体.
	while (nr > 0) {
		...
		while (nr > 0 && !FULL(tty->write_q)) {
			...
			// 接着把用户数据缓冲指针 b 前移 1 字节; 欲写字节数减 1 字节; 复位 cr_flag 标志, 并将该字节放入 tty 写队列中.
			b++; nr--;
			cr_flag = 0;
			PUTCH(c, tty->write_q);
		}
		// 若要求的字符全部写完, 或者写队列已满, 则程序退出循环. 此时会调用对应 tty 写函数, 把写队列缓冲区中的字符显示在控制台屏幕上, 或者通过串行端口发送出去. 
		// 如果当前处理的 tty 是控制台终端, 那么 tty->write() 调用的是 con_write(); 如果 tty 是串行终端, 则 tty->write() 调用的是 rs_write() 函数. 
 ...
 */
/*
 * 呵, 有时我真的是很喜欢 386. 该子程序被从一个中断处理程序中调用, 并且即使在中断处理程序中睡眠也应该绝对没有问题(我希望如此). 
