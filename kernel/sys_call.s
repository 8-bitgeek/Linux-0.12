...
ret_from_sys_call:
	...
	# 通过对当前进程的代码段选择符的检查来判断是否处于用户态. **如果是内核态则直接退出中断**.
	# 这是因为任务在内核态执行时不可抢占(不能执行其它任务, 即不能执行后面的 reschedule, 不过进程可以睡眠并主动让出 cpu, 
	# 比如读取硬盘数据时就处于内核态, 但是进程会主动调用 schedule() 来进入睡眠并让出 cpu). 
	# 如果被中断进程处于用户态, 则对信号进行处理.
	...
	# 如果当前进程是处于用户态(且不是 TASK-0), 则处理当前任务中的信号. 
