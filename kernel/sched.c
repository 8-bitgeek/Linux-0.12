...
void schedule(void)
{
	...
	// next 被初始化为 0. 因此若系统中没有任何其它任务处于就绪状态, 则 next 始终为 0. 
...
static inline void __sleep_on(struct task_struct **p, int state)
{
	...
	tmp = *p; 													// tmp 指向之前就已经在等待资源的任务指针(如果有的话).
	*p = current; 												// 更新睡眠等待资源的任务指针为当前任务指针.
	current->state = state; 									// 更新当前任务状态.
	// 将当前进程睡眠后立刻执行调度函数调度其它的进程执行.
repeat:	schedule();
	// 只有当这个等待任务被唤醒时, 程序才会返回到这里, 表示进程已被明确地唤醒(state 更新为 0?)并执行. 
	// 如果等待队列中还有等待任务, 并且队列头指针 *p 所指向的任务不是当前任务时,
	// 说明在本任务插入等待队列后还有任务进入等待队列. (TODO: 这个逻辑有问题)
	// 于是我们应该也要唤醒这个任务, 而我们自己应按顺序让这些后面进入队列的任务唤醒, 
	// 因此这里将等待队列头所指任务先置为就绪状态, 而自己则置为不可中断等待状态, 
	// 即自己要等待这些后续队列的任务被唤醒而执行时来唤醒本任务, 然后重新执行调度程序.
	if (*p && *p != current) {
		(**p).state = 0; 										// 将其它任务的状态更新为就绪状态.
		current->state = TASK_UNINTERRUPTIBLE; 					// 当前任务为不可中断等待状态.
		goto repeat;											// 重新调度, 让出 CPU 让其它进程先运行.
	}
	// 执行到这里, 说明本任务真正被唤醒执行. 
	// 此时等待队列头指针应该指向本任务, 若它为空, 则表明调度有问题, 于是显示警告信息. 
	// 最后我们让头指针指向在我们前面等待资源的任务(*p = tmp). 
