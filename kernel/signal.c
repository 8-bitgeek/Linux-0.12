	...
	unsigned long * esp, long ss)
{
	...
	if (!sa_handler) {
		...
		// 若当前进程父进程对 SIGCHLD 信号的 sigaction 处理标志 SA_NOCLDSTOP(即当子进程停止执行或又继续执行时不要产生 SIGCHLD 信号)没有置位, 
	...
	*(&eip) = sa_handler;
	...
	// 在用户堆栈中从下到上存放 sa_restorer, 信号 signr, 屏蔽码 blocked(如果 SA_NOMASK 置位), eax, ecx, edx, eflags 和用户程序原代码指针. 
	tmp_esp = esp;
