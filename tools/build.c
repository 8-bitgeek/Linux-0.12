 ...
 * 然后删除模块头部并扩充到正确的长度. 该程序也会将一些系统数据写到 stderr.
 */
/*
...
// 如果 build 程序命令行参数个数不是 4 到 6 个(程序名算作 1 个), 则显示程序用法并退出.        
	if ((argc < 4) || (argc > 6))
		usage();
...
// 如果最后一个参数就是"NONE", 则让交换设备的主设备号和次设备号取为 0. 表示交换设备就是当前启动引导设备.
	if (argc == 6) {
...
// 若参数没有 6 个而是 5 个, 表示命令行上没有交换设备名. 于是就让交换设备主设备号和次设备号等于系统默认的交换设备号.               
	} else {
		major_swap = DEFAULT_MAJOR_SWAP;
		minor_swap = DEFAULT_MINOR_SWAP;
	}
// 接下来在标准错误终端上显示上面所选择的根设备主, 次设备号和交换设备主, 次设备号. 
// 如果主设备号不等于 2(软盘)或 3(硬盘) 并且也不为 0(取系统默认设备), 则显示出错信息并退出. 
...
// 接下来根据 MINIX 头部结构判断 setup 是否为一个有效的 MINIX 执行文件. 若是, 则从文件中读取 512 字节的引导扇区代码和数据.
// 0x0301 - MINIX 头部 a_magic 魔数; 0x10 - a_flag 可执行; 0x04 - a_cpu, Intel 8086 机器码.        
	if (((long *) buf)[0] != 0x04100301)
...
// 在上述判断都正确的条件下读取文件中随后的实际代码数据, 并且写到终端标准输出. 同时统计写的长度(i), 
// 该值不能大于(SETUP_SECTS * 512)字节, 否则就得重新修改 build, bootsect 和 setup 程序中设定的 setup 所占扇区数并重新编译内核. 
...
// 首先以只读方式打开文件并读取其中 a.out 格式头部结构信息(1KB 长度). 
// 在判断 system 是一个有效的 a.out 格式文件之后, 就把该文件随后的所有数据都写到标准输出(Image 文件)中, 并关闭该文件. 
// 然后显示 system 模块的长度. 若 system 代码和数据长度超过 SYS_SIZE 字节(即 128KB)字节, 则显示出错信息并退出. 若无错, 则返回 0, 表示正常退出.        
