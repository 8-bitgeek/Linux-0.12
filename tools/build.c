...
int main(int argc, char ** argv)
{
	...
	// 若参数没有 6 个而是 5 个, 表示命令行上没有交换设备名. 
	// 于是就让交换设备主设备号和次设备号等于系统默认的交换设备号.               
	} else {
	...
	// 在上述判断都正确的条件下读取文件中随后的实际代码数据, 并且写到终端标准输出. 
	...
	// 在判断 system 是一个有效的 a.out 格式文件之后, 
	// 就把该文件随后的所有数据都写到标准输出(Image 文件)中, 并关闭该文件. 
	// 然后显示 system 模块的长度. 
	// 若 system 代码和数据长度超过 SYS_SIZE 字节(即 128KB)字节, 则显示出错信息并退出. 
	// 若无错, 则返回 0, 表示正常退出.        
